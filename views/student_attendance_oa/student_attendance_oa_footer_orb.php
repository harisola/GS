...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            <?php if($this->data['can_user_view'] == 1 && $this->data['can_user_edit'] == 1 && $this->data['can_user_delete'] == 0 && $this->uri->uri_string() == 'student_attendance_oa/atd_today') {?>
            if ($('#class_list_view_faculty_table').length) {
                var oTable = $('#class_list_view_faculty_table').dataTable({
                    ...
                    "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                        $('.aneditable_attendance_present a').editable({
                            inputclass: 'input-large',
                            source: [
                                {value: '07:30:00', text: 'Present'}
                            ],
                            url: '<?php echo base_url()?>index.php/student_attendance_oa/atd_today/edit2',                
                            validate: function(value) {
                               if($.trim(value) == '') return 'This field is required';                   
                            }   
                        });

                        $('.aneditable_attendance_absent a').editable({
                            inputclass: 'input-large',
                            source: [
                                {value: '00:00:00', text: 'Absent'}                                
                            ],
                            url: '<?php echo base_url()?>index.php/student_attendance_oa/atd_today/edit',                
                            validate: function(value) {
                               if($.trim(value) == '') return 'This field is required';                   
                            }   
                        });
...
$(document).on("click", ".show_student_information", function (e) {
    ...
    $.post('<?php echo base_url() . "index.php/student_attendance_oa/student_profile_basic_view_absent/";?>?grno='+GR_No, function(data) {
...
$(function() {
    $("#confirm_tpa").click( function()
        {
            var grade_section = $("#selected_grade").val()+','+$("#selected_section").val();
            if($("#selected_grade").val() == '0'){
                alert("Please select Grade");
            }else{
                if (confirm('Are you sure to confirm TPA of '+$("#selected_grade option:selected").text()+'-'+$("#selected_section option:selected").text()+'?')) {
                    $.ajax({        
                       type: "POST",
                       url:"<?php echo site_url()?>/student_attendance_oa/atd_today/add3",
                       data: "selected_grade_section="+grade_section,
                       success: function() {
                            alert('TPA confirmed of '+$("#selected_grade option:selected").text()+'-'+$("#selected_section option:selected").text());
                       }
                    });                    
                }
            }            
