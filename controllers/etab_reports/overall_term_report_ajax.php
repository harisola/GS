...
class Overall_term_report_ajax extends CI_Controller{
    ...
    public function get_overall_term_report_combine_pdf(){
    ...
    if($gradeID >= 6 &&  $gradeID <= 9){
        ...
        $pdf_link = '/etab_reports/overall_term_report_ajax/overall_term_report_combine_junior?
        gradeID=' . $gradeID .
        '&sectionID=' . $sectionID .
        '&gradeName=' . $gradeName .
        ...
        $pdf_link = '/etab_reports/overall_term_report_ajax/overall_term_report_combine?
        gradeID=' . $gradeID .
        '&sectionID=' . $sectionID .
        '&gradeName=' . $gradeName .
        '&sectionName=' . $sectionName .
    ...
    public function overall_term_report_combine(){
        ...
        /******************************** Students Data 
        /***********************************************/
        $data['students_term1'] = $this->assessment_report_model->get_complete_marks_of_ACD("1718", $gradeID, 1);
        /***********************************************/
        ...
        if($termID == 2){
            if($gradeID == 10 || $gradeID == 11){
                ...
                $data['assessment_titles'] = $this->assessment_report_model->Complete_Assessment_Report_GS_Subjects_SessionTerm($gradeID, $sectionID, "_".$session_year."_2");
            }
        }else{
            if($gradeID == 10 || $gradeID == 11){
                ...
                //$data['assessment_titles'] = $this->assessment_report_model->Complete_Assessment_Report_GS_Subjects($gradeID, $sectionID);
                $data['assessment_titles'] = $this->assessment_report_model->Complete_Assessment_Report_GS_Subjects_SessionTerm($gradeID, $sectionID, "_".$session_year."_1");
            }
        }

        $data['all_subjects'] = $this->assessment_report_model->Complete_Assessment_Report_GS_Subjects_Split($gradeID, $sectionID, "_".$session_year."_2");
        ...
        /***********************************************/
        $data['assessment_overall_garde'] = $this->assessment_report_model->get_Grade_Overall_grading($gradeID, "_".$session_year);
        /*************************************************************************************************************************************/
        ...
        if($termID == 2){
            if($session_year == "1617"){
                $TermReportTitle = ' (Overall Term Report 2016-17)';
            }else if($session_year == "1718"){
                $TermReportTitle = ' (Overall Term Report 2017-18)';
            }
        }else{
            $TermReportTitle = ' (Overall Term Report 2016-17)';
        }
        ...
        foreach ($data['students_gs_data'] as $student) {
            ...
            $studentMarks[$student->gs_id]['ft_grade_u'] = 0;
            $studentMarks[$student->gs_id]['st_grade_u'] = 0;
            $studentMarks[$student->gs_id]['ftst_grade_u'] = 0;
                    ...
                    if($marks->gs_id == $student->gs_id
                       && $marks->subject_name == $title->subjectname){
                        if($marks->assessment_type_id == 1 && $marks->ass_per_mrk > 0){
                            ...
                            if($marksObtained_ft < $data['assessment_overall_garde'][5]->weightage){
                                $studentMarks[$student->gs_id]['ft_grade_u']++;
                            }
                            ...
                            if($marksObtained_st < $data['assessment_overall_garde'][5]->weightage){
                                $studentMarks[$student->gs_id]['st_grade_u']++;
                            }
                    ...
                    if($grade->gs_id == $student->gs_id
                       && $grade->subject_name == $title->subjectname){
                        ...
                        if($grade->grade == 'U'){
                            ...
                            $studentMarks[$student->gs_id]['ftst_grade_u']++;
                        }
                    }
                }
            ...
            if($studentMarks[$student->gs_id]['ft_grade_u']==0){$studentMarks[$student->gs_id]['ft_grade_u']='';}
            if($studentMarks[$student->gs_id]['st_grade_u']==0){$studentMarks[$student->gs_id]['st_grade_u']='';}
            if($studentMarks[$student->gs_id]['ftst_grade_u']==0){$studentMarks[$student->gs_id]['ftst_grade_u']='';}
        ...
        foreach ($data['assessment_titles'] as $title) {
            ...
            foreach ($data['students_gs_data'] as $student) {
                ...
                foreach ($data['students_term1'] as $term1) {
                    if($term1->gs_id == $student->gs_id){
                        $studentTerm1Perc[$student->gs_id] = $term1->percentage;
                        $sub = 0;
                        $tot = 0;
                        $perc = 0;
                        if(!empty($studentMarks[$student->gs_id]['subjects'])){
                            $sub = sizeof($studentMarks[$student->gs_id]['subjects']);
                            $tot = $studentTotalMarks[$student->gs_id];
                            $perc = ROUND(($tot / $sub), 1);                            
                        }                       
                        $studentOverallPerc[$student->gs_id] = ROUND(($studentTerm1Perc[$student->gs_id] + $perc)/2, 1);
                    }
                }
            }
        }

        //var_dump($data['all_subjects']);
        //exit;
        /***********************************************************************************************/
        ...
        if(($startWith_X+$TotalWidthUsed) <= $pdf->w-10){
            ...
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width, $table_header_height, 'Term-1',1,0,'C', 1);

            $startWith_X += $student_h_width;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width, $table_header_height, 'Term-2',1,0,'C', 1);

            $startWith_X += $student_h_width;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width, $table_header_height, 'Average',1,0,'C', 1);

            $startWith_X += $student_h_width;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width, $table_header_height, 'Grade',1,0,'C', 1);

            $startWith_X += $student_h_width;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width, $table_header_height, 'Rank',1,0,'C', 1);

            $startWith_X += $student_h_width;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width/2, $table_header_height, 'F',1,0,'C', 1);

            $startWith_X += $student_h_width/2;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width/2, $table_header_height, 'S',1,0,'C', 1);

            $startWith_X += $student_h_width/2;
            $pdf->SetFont($table_header_font_name,'B',$table_header_font_size);
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($student_h_width/2, $table_header_height, 'C',1,0,'C', 1);
            ...
            $startWith_Y = $dataFlowY;
            $startWith_X -= ($student_h_width*(($no_of_columns)+7));
            ...
            foreach ($data['students_gs_data'] as $student) {
                ...
                if(!empty($studentMarks[$student->gs_id]['subjects'])){
                    ...
                    if($studentOverallPerc[$student->gs_id] >= $grade_Ap_per){
                        $thisText_Grade_Overall = 'A+';
                    }else if($studentOverallPerc[$student->gs_id] >= $grade_A_per){
                        $thisText_Grade_Overall = 'A';
                    }else if($studentOverallPerc[$student->gs_id] >= $grade_B_per){
                        $thisText_Grade_Overall = 'B';
                    }else if($studentOverallPerc[$student->gs_id] >= $grade_C_per){
                        $thisText_Grade_Overall = 'C';
                    }else if($studentOverallPerc[$student->gs_id] >= $grade_D_per){
                        $thisText_Grade_Overall = 'D';
                    }
                ...
                $OverAll_Perc[$OverAll_Perc_count] = $thisText_Per; 
                ...
                //Horizontal Overall Term
                $thisStudentOverall_Rank = $this->assessment_report_model->getRankOf($studentOverallPerc, $studentOverallPerc[$student->gs_id]);


                if($thisText_Grade_Overall!='U'){
                    $pdf->SetFillColor(230,230,230);
                    $pdf->setTextColor(0,0,0);
                }
                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width, $text_column_height, $studentTerm1Perc[$student->gs_id],1,0,'C',1);
                ...
                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width, $text_column_height, $studentOverallPerc[$student->gs_id],1,0,'C',1);


                if($thisText_Grade_Overall!='U'){$pdf->SetFillColor(255,255,255);}
                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width, $text_column_height, $thisText_Grade_Overall,1,0,'C',1);

                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width, $text_column_height, $thisStudentOverall_Rank,1,0,'C',1);

                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width/2, $text_column_height, $studentMarks[$student->gs_id]['ft_grade_u'],1,0,'C',1);

                $startWith_X += $student_h_width/2;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width/2, $text_column_height, $studentMarks[$student->gs_id]['st_grade_u'],1,0,'C',1);

                $startWith_X += $student_h_width/2;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width/2, $text_column_height, $studentMarks[$student->gs_id]['ftst_grade_u'],1,0,'C',1);
            ...
            *****************************************************************************************/
            ...
            $pdf->SetFillColor(230,230,230);
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y);
            $pdf->Cell(($student_h_width*3), $text_column_height, 'Class Teacher', 1, 0, 'C', 1);
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y);
            $pdf->Cell(($student_h_width*3), ($text_column_height*3), '', 1, 0, 'C');
            $pdf->SetFillColor($BG_text_fontColor_r, $BG_text_fontColor_g, $BG_text_fontColor_b); 
            ...
            $pdf->SetFillColor(230,230,230);
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y);
            $pdf->Cell(($student_h_width*3), $text_column_height, 'Headmistress', 1, 0, 'C', 1);
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y);
            $pdf->Cell(($student_h_width*3), ($text_column_height*3), '', 1, 0, 'C');
            /************************************** Signature - Principal *****/
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y+($text_column_height*3));
            $pdf->Cell(($student_h_width*3), $text_column_height, 'Principal', 1, 0, 'C', 1);
            $pdf->SetXY($startWith_X+($student_h_width*3),$startWith_Y+($text_column_height*3));
            $pdf->Cell(($student_h_width*3), ($text_column_height*3), '', 1, 0, 'C');
            $pdf->SetFillColor($BG_text_fontColor_r, $BG_text_fontColor_g, $BG_text_fontColor_b);
            ...
            ************************************************************************************************/
            ...
            foreach ($data['students_gs_data'] as $student) {
                ...
                $i++;
                $startWith_X -= $text_column_width_status + ($student_h_width * ($no_of_columns+7)) + 5;
                $startWith_Y = $startWith_Y - ($table_header_height-$text_column_height);
                $OverAll_Perc_count++;
            }
            ...
            ************************************************************************************************/
            ...
            /*$startWith_X += $student_h_width/2;
            $pdf->SetXY($startWith_X,$startWith_Y);
            $pdf->Cell($text_column_width_abname, $table_header_height, 'Abridged Name',1,0,'L', 1);*/
        ...
        // Output the new PDF
        $pdf->Output('Overall_Term_Report' . '.pdf', 'I');
    }


    // public function overall_term_report_combine_junior(){
    //     $sectionID = $this->input->get('sectionID');
    //     $gradeID = $this->input->get('gradeID');
    //     $gradeName = $this->input->get('gradeName');
    //     $sectionName = $this->input->get('sectionName');
    ...
    //             if($grading->grading_name == 'A'){$grade_A_per = $this->assessment_report_model->getCountOf_GreaterOnly_2d_Between($OverAll_Perc, 85, 75);}
    //             if($grading->grading_name == 'B+'){$grade_B_per = $this->assessment_report_model->getCountOf_GreaterOnly_2d_Between($OverAll_Perc, 75, 65);}
    //             if($grading->grading_name == 'B'){$grade_C_per = $this->assessment_report_model->getCountOf_GreaterOnly_2d_Between($OverAll_Perc, 65, 55);}
    //             if($grading->grading_name == 'C+'){$grade_D_per = $this->assessment_report_model->getCountOf_GreaterOnly_2d_Between($OverAll_Perc, 55, 50);}
     ...
     public function overall_term_report_combine_junior(){
        ...
        foreach ($data['students_gs_data'] as $student) {
            ...
            foreach ($data['assessment_titles'] as $title) {
                foreach ($data['assessment_marks'] as $marks) {
                    $marksObtained_ft = 0; $marksObtained_st = 0;
                    if($marks->gs_id == $student->gs_id && $marks->subject_name == $title->subjectname){
                        if($marks->assessment_type_id == 1 && $marks->ass_per_mrk > 0){
                            $marksObtained_ft = $marks->ass_per_mrk;
                            //var_dump($marksObtained_ft);
                            $Total+=$marksObtained_ft;
                ...
                foreach ($data['assessment_grade'] as $grade) {
                    if($grade->gs_id == $student->gs_id && $grade->subject_name == $title->subjectname){
                        $studentMarks[$title->subjectname]['gd'][$student->gs_id] = $grade->grade;
                        $studentTotalMarks[$student->gs_id] += $grade->ass_per_mrk;
                        if($grade->grade == 'C'){
                            $studentGrade_U[$student->gs_id]=1;
        ...
        if(($startWith_X+$TotalWidthUsed) <= $pdf->w-100){
            $startWith_X += 5;
            $startWith_Y = $ReportStarting_Y+20;
            ...
            foreach ($data['students_gs_data'] as $student) {
                ...
                if($thisText_Grade_Overall!='C'){$pdf->SetFillColor(255,255,255);}
                $startWith_X += $student_h_width;
                $pdf->SetXY($startWith_X,$startWith_Y);
                $pdf->Cell($student_h_width, $text_column_height, $thisText_Grade_Overall,1,0,'C',1);
