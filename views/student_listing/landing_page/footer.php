...
<div class="modal fade" id="student_activecase_modal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">Active Case</h4></div>
      <div id="modal_body_activecase" class="modal-body text-center ativa-scroll"></div>                                       
...
#ajaxloader3{background-color: silver}
#ajaxloader3{
position: relative;
border:none;
...
<!-- Bedges Modal -->
<div class="modal fade TimeLineModal" id="bedge_detail" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
			...
			<div style="display:none;" id="ajaxloader3">  <p> <br /> <br /> <br /> <br /> Please Wait .. </p></div>
			 <div class="modal-body" id="bedge_detail_body">
...
$(document).on("click", ".tab_action", function(){
	...
	if( student_id > 0 ){
        $.ajax({
			...
			data:{tab_id:tab_id,student_id:student_id,gs_id:gs_id,gr_no:gr_no,gf_id:gf_id},
			dataType:"HTML",
...
function AddRomveSelected() {
	/*var table = $('#EntitityListing');    
	...
	if ( $(this).hasClass('selected') ) {
	 ...
	 $("#EntitityListing tr").each(function() {
 ...
 $(document).on("click",".anchorCol", function(){
	 var sid = $(this).prop("id");
	  var sid2 = $(this).prop("id");
	 var sid = sid.split("std_");
	 var std_id = parseInt( sid[1] );
	 
	  var sid2 = sid2.split("_gsid_");
	 var gsid =  sid2[1];
	 //alert(gsid)
	//AddRomveSelected();
	...
	$(this).closest('tr').addClass('selected');
	 ...
	 $("#ajaxloader2").show();
	 var tab_id = $("#active_tab").val();
	 if( std_id > 0 ){
        $.ajax({
            type: "POST",
            url: "<?=base_url();?>index.php/student_listing/tabs/student_info",
			beforeSend: function(){  $("#ajaxloader2").show(); },
			data:{student_id:std_id,tab_id:tab_id,gsid:gsid},
			dataType:"HTML",
			success: function(res){
               $("#sectionRight").html('');
			   //$("#sectionRight").html(res);
			   $("#sectionRight").html(res).fadeIn(500);
			  // console.log(res)
			},
			complete:function(){ 
			$("#ajaxloader2").hide(); 
...
$(document).on("click", ".show_student_activecase", function (e) {
    e.preventDefault();
    var _self = $(this);
    var gsid = _self.data('id');    
    $.post('<?php echo base_url() . "index.php/student_attendance/activecase_current_modal/index_Index/";?>?gsid='+gsid, function(data) {
      $('#modal_body_activecase').html(data);
    });
    $("#student_activecase_modal").modal('show');
});

</script>
<script>

$(document).on("click", ".AddNewBadge", function (e) { 
//$("#ListBadges").hide();
...
$(document).on("click", "#BackBadge", function () { 
$("#ListBadges").show();
$("#AddNewBadgeArea").hide();
Load_Badge_List();
});

function Load_Badge_List(){
	var Grade_id = $("#hidden_grade_id").val();
var Section_id = $("#hidden_section_id").val();
var Current_Term = $("#hidden_Current_Term").val();
...
$.ajax({
	type: "POST",
	url: "<?=base_url();?>index.php/student_listing/badges_managment/Badges_List",
	beforeSend: function(){  $("#ajaxloader3").show(); },
	data:{Grade_id:Grade_id,Section_id:Section_id},
	dataType:"HTML",
	success: function(res){
	   $("#bedge_detail_body").html('');
	   $("#bedge_detail_body").html(res);
...
$(document).on("click", "#BadgeAlloc", function(){
Load_Badge_List();
});


function Add_New_Badge_Form(){

var Grade_id = $("#hidden_grade_id").val();
var Section_id = $("#hidden_section_id").val();
var Current_Academic = $("#hidden_Current_Academic").val();
var Current_Term = $("#hidden_Current_Term").val();
...
$.ajax({
	type: "POST",
	url: "<?=base_url();?>index.php/student_listing/badges_managment/Create_Badges",
	beforeSend: function(){  $("#ajaxloader3").show(); },
	data:{Grade_id:Grade_id,Section_id:Section_id,Current_Term:Current_Term,Current_Academic:Current_Academic},
	dataType:"HTML",
	success: function(res){
		$("#ajaxloader3").hide(); 
	   $("#bedge_detail_body").html('');
...
$(document).on("click", ".AddNewBadge", function(){ 
//$('#bedge_detail').modal('hide');
Add_New_Badge_Form();
});


// Create Badges
$(document).on("click", "#create_badge", function(){

	$("#issuance").validate({
		rules: {
			badge_title: { required: true	},
			badge_code: { required: true	},
			//badge_expiry: { required: true	},
			badge_color: { required: true	}
		//badge_priority: { required: true	}
		},
		messages: {
			badge_title: { required: 'Badge Title' },
			badge_code: { required: 'Badge Code' },
			//badge_expiry: { required: 'Badge Code' },
			badge_color: { required: 'Badge Color' }
		//badge_priority: { required: 'Badge Priority' }
		},
...
if( Grade_id > 0 ){
	$.ajax({
		type: "POST",
		url: "<?=base_url();?>index.php/student_listing/badges_managment/Edit_Badge",
		beforeSend: function(){  $("#ajaxloader3").show(); },
		data:{Badge_id:Badge_id,Grade_id:Grade_id,Section_id:Section_id,Current_Academic:Current_Academic,Current_Term:Current_Term},
		dataType:"HTML",
		success: function(res){
			$("#bedge_detail_body").html('');
			$("#bedge_detail_body").html(res);
...
$(document).on("click", "#btn_edit_badge", function(){
...
var Updation_Badge = $("#hidden_updation_Badge").val();


if( Updation_Badge > 0 ){
	$.ajax({
		type: "POST",
		url: "<?=base_url();?>index.php/student_listing/badges_managment/student_table_list",
		beforeSend: function(){  $("#ajaxloader").show(); },
		data:{Grade_id:Grade_id,Section_id:Section_id,Current_Academic:Current_Academic,Current_Term:Current_Term},
		dataType:"HTML",
		success: function(res){
			$("#grade_section_list_table").html('');
			$("#grade_section_list_table").html(res);
			$("#hidden_updation_Badge").val(0);
			$("#ajaxloader").hide(); 
		},
		complete:function(){ 
...
$(document).on("click", ".floatClose",   function(){
Reload_Student_Table();
});
</script>
