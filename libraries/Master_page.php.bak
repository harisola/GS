...
class Master_page{
	...
	function mp_menus(){
		...
		$html .= '<li class="hasSubChild">';
		$html .= '<a href="#">My Class Groups<i class="rightArrow fa fa-angle-right " aria-hidden="true"></i></a>';
		$html .= '<ul class="sub-subMenu">';
		foreach($teacherSubjects2 as $g ){
			...
			$encode_url = 'user_id='.$reg_user_id.'&matrixRoleSbjTchrID='.$tSID.'&programmeID='.$pID.'&secName='.$secName.'&section_id='.$sectionID.'&grade_id='.$gradeID.'&Grade='.$Grade.'&subjectID='.$subjectID.'&Subject='.$Subject.'&subjectCode='.$subjectCode.'&optional='.$OPT.'&GPID='.'&gpp_id='.$gpp_id.'';
			
			
			
			$html .= '<li><a href="'.site_url().'/student_listing/grade_student_info?'.$encode_url.'">'.$GPID.'</a> </li>';
	...
	public function mp_prgAssmnt( $proID, $session,$term ){
		$this->ci->load->model('etab/teacher_side/teacher_side_model', 'ETS');
