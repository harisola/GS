...
$(document).ready(function(){
    ...
    $('#student_timeline_options_div').hide();

    $(document).on('click', '#student_log_grade_section_table tr', function (event) {

        var selected = $(this).hasClass("highlight1");
        $("#student_log_grade_section_table tr").removeClass("highlight1");
        if(!selected)
    ...
    $(document).on('click', '#student_log_grade_section_std tr', function (event) {

        var selected = $(this).hasClass("highlight1");
        $("#student_log_grade_section_std tr").removeClass("highlight1");
        if(!selected)
            $(this).addClass("highlight1");


        /*************************************/
        // checkbox time line options
        var selectedTimeLine = '';
        $('#student_timeline_all_options input:checked').each(function() {
            selectedTimeLine = selectedTimeLine + "log_name like '%"+$(this).attr('value')+"%' or ";
        });
        selectedTimeLine = selectedTimeLine.substring(0,selectedTimeLine.length - 4);
        /*************************************/


        var $row = $(this).closest("tr");    // Find the row
        var $gsID = $row.find(".studentID").text(); // Find the text
        var $selectedOption = $('#student_complete_information li.active').text();
        $selectedOption = $.trim($selectedOption);

        $.ajax({
            type: "POST",
            url: "<?php echo site_url() . '/ajax/student_log_gsstudents/getStudentInformation';?>",
            data: {
                gsid: $gsID,
    ...
    $(document).on('click', '#student_complete_information li', function (event){
        var $row = $('#student_log_grade_section_std tr.highlight1').closest("tr");    // Find the row
        var $gsID = $row.find(".studentID").text(); // Find the text
        var $selectedOption = $('#student_complete_information li.active').text();
        $selectedOption = $.trim($selectedOption);
            ...
            success: function(data){
                ...
                if(data.length > 3){
                    ...
                    if($selectedOption == 'Time Line'){
                        $.ajax({
                          type: "POST",
                          url: "<?php echo site_url() . '/ajax/student_log_gsstudents/get_TimeLine_Filters';?>",
                          success: function(data){
                              //alert(data);
                              if(data.length > 3){
                                $('#student_timeline_all_options').html(data);
                              }
                          },
                          error: function() {
                      ...
                      $('#student_timeline_options_div').show();
                    }else{
                      $('#student_timeline_options_div').hide();
                    }
                }
    ...
    var container = $('#student_log_grades');
    var scrollTo = $('.highlight1');
...
$("#student_log_grade_section_table").on('click', 'tr', function (event) {
    event.preventDefault();
    var $row = $(this).closest("tr");    // Find the row
    var $gradeSection = $row.find(".grade_section").text(); // Find the text
    

    $.ajax({
        type: "POST",
        url: "<?php echo site_url() . '/ajax/student_log_gsstudents/getStudentsList';?>",
        data: {
...
$(document).on('click', '#timeline_show_link', function (event){      
    var TotalRecords = $('#timeline_records').val();
    var GSID = $('#timeline_gsid').val();
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo site_url() . '/ajax/student_log_gsstudents/get_ThisStudent_LimeLine';?>",
        data: {
            recordFrom: TotalRecords,
            GSID: GSID,
            selectedTimeLine: selectedTimeLine
        },
        success: function(data){
            //alert(data);
            if(data.length > 3){
                data_li = data.substring(0, data.length - 1);
                data_last = data.substring(data.length, data.length - 1);
                $(".activity-block ul").append(data_li);


                if(data_last == '1'){
                  TotalRecords = parseInt(TotalRecords)+10;
                  $("#timeline_more_records").html('Displaying ' + TotalRecords + ' records, <input type="hidden" id="timeline_records" value="' + TotalRecords + '"> <input type="hidden" id="timeline_gsid" value="' + GSID + '"> <a id="timeline_show_link" href="#">show more?</a>');
                }else{
                  $("#timeline_more_records").html('Displaying ' + TotalRecords + '+ records.');
                }


                $('#id_student_timeline').animate({scrollTop: $('#id_student_timeline').get(0).scrollHeight}, 2000);
            }
        },
        error: function() {
...
/**************************************************************************************************/
$(document).on('change', '#student_timeline_all_options input[type=checkbox]', function (event){
    var $row = $('#student_log_grade_section_std tr.highlight1').closest("tr");    // Find the row
    var $gsID = $row.find(".studentID").text(); // Find the text
    var $selectedOption = $('#student_complete_information li.active').text();
    $selectedOption = $.trim($selectedOption);
...
function showComments(commentsID)
{
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo site_url() . '/ajax/student_log_gsstudents/get_these_comments';?>",
        data: {
...
function saveComments()
{
    ...
    var arr = [];
    $('.multi_select_comment_type :selected').each(function(i, selected){ 
        arr.push(this.value); 
    });
    var tags = arr.join(', ');
    ...
    if(tags=='' || comments==''){
      if(tags==''){
        $().toastmessage('showErrorToast', "Please select atleast one tag ...");
      }else if(comments==''){
        $().toastmessage('showErrorToast', "Please enter few words in comments ...");
      }
    }else{
      $.ajax({
        type: "POST",
        url: "<?php echo site_url() . '/ajax/student_log_gsstudents/save_these_comments';?>",
        data: {
            tag: tags,
            comments: comments,
            GSID: gsID
        ...
        success: function(data){
            ...
            $().toastmessage('showSuccessToast', "Comments added ...");
            $('#comments_text').val('');
        },
        error: function() {
...
<script type="text/javascript" src="<?php echo base_url();?>components/orb/js/vendors/fullcalendar/fullcalendar.min.js"></script> 
<script type="text/javascript" src="<?php echo base_url();?>components/orb/js/vendors/fullcalendar/gcal.js"></script>




<link type="text/css" href="<?php echo base_url();?>components/toast/resources/css/jquery.toastmessage.css" rel="stylesheet"/>
<script type="text/javascript" src="<?php echo base_url();?>components/toast/js/jquery.toastmessage.js"></script>
