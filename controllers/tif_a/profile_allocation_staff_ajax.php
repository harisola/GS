...
class Profile_allocation_staff_ajax extends CI_Controller{
	...
	public function allocate_std_staff() {
		...
		$is_on_flexy = $profile_detail[0]->is_on_flexy;
		$is_on_relaxation = $profile_detail[0]->is_on_relaxation;

		//=======================================//
		  // Saturday Working Hours Calculation
		//=======================================//
		...
		if($is_on_flexy == 1){
			$flexy_time = $profile_detail[0]->flexy_time;
		}else{
			$flexy_time = '00:00:00';
		}
		...
		if($is_on_relaxation == 1){
			$relaxation_time = $profile_detail[0]->relaxation_time;
		}else{
	    ...
	    if($sat_hour != 0 && ($sat_off != 0 || $sat_on != 0)){
		  	$is_on_sat = 1;
		  	$sign = ($sat_hour < 0 ? "-" : "");
    ...
    //======================================================//
       ...
       if($profile_detail[0]->ext_july != '1970-01-01'){
       ...
       if($profile_detail[0]->sat_working != 0){
	    ...
	    if($profile_detail[0]->is_on_flexy == 1){
	    	 $html .= '<input type="time" value="'.$profile_detail[0]->flexy_time.'" disabled/>';
	    }else{
	    ...
	    $html .= '</div><!-- col-md-4 -->';
	    $html .= '<div class="col-md-8 no-padding">';
	    if($profile_detail[0]->is_on_relaxation == 1){
	    	$html .= '<input type="time" value="'.$profile_detail[0]->relaxation_time.'" disabled />';
	    }else{
	    	$html .= '<input type="time" disabled />';
	    }   
	    $html .= '</div><!-- col-md-8 -->';
    ...
    //=============================================================//
		...
		$html .= '<td width="" colspan="2" style="border-right: 1px solid #666;" bgcolor="#b5d9e6"><strong>Sat</strong></td>';
		$html .= '</tr>';
