...
class Notify_log_ajax extends CI_Controller {
	...
	public function get_converssion(){
		...
		$tableRows=array();
		if( $tableRow = $this->ABM->gt_ntfctn_gs_id_wise($notify_to, $cat_id, $staff_id, $gs_id) ){
			$tableRows = $tableRow;
		...
		$thread_id = 0;
		$CaseName = "";
		if( $tid = $this->ABM->get_thread_id($gs_id) ){
          ...
          $this->data['photo_file'] = '.png';
           ...
           $html .= $ActiveCaseInfo;
							...
							/*if( !empty( $tableRow ) ){
										...
										$photo_url = base_url()."/assets/photos/hcm/150x150/staff/".$tr["S_emp_id"].".png";
										
										if( $tr["N_register_by"] == $staff_id){
											$html .= '<li class="right clearfix2">';	
											$html .= '<div class="user-img pull-right">';
										}else{
											$html .= '<li class="left clearfix2">';	
											$html .= '<div class="user-img pull-left">';
										}
										 
										 
										$html .= '<input type="hidden" class="N_log_id" value="'.$tr["N_log_id"].'" />';
										$html .= '<input type="hidden" class="N_cat_id" value="'.$tr["N_cat_id"].'" />';
										$html .= '<input type="hidden" class="N_type" value="1" />';
										
										$html .= '<input type="hidden" class="gs_id" value="'.$tr["gs_id"].'" />';
										$html .= '<input type="hidden" class="notify_to" value="'.$tr["notify_to"].'" />';
										$html .= '<input type="hidden" class="S_id" value="'.$tr["S_id"].'" />';
										$html .= '<img src="'.$photo_url.'" alt="'.$tr["S_name"].'" />';
										  $html .= '</div>';
                                          $html .= '<div class="chat-body clearfix">';
                                          $html .= '<div class="header">';
						  				  $html .= '<span class="name">'.$tr["S_name"].'</span>';
										  $html .= ' <small>'.$tr["S_desgntn"].'</small>';
										  $html .= ' <small>'.$tr["S_deprt"].'</small>';
										  $html .= '<span class="badge">';
										  $html .= '<i class="fa fa-clock-o"></i>'.date("Y-m-d",$tr["N_create_date"]).'</span></div>';
                                          $html .= '<p>'.$tr["N_message"].'</p>';
                                          $html .= '</div>';
										  $html .= '</li>';
										}
										}
