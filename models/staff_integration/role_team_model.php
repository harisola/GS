<?php
class Role_team_model extends CI_Model{
	...
	protected $_table_name = 'role_team';
	...
	public function generateRoleTeamID()
	{
		$query=$this->db->query("SELECT max(gm_id) as max_id FROM role_team"); 
		...
		if ($query->num_rows() > 0) {
			...
			$max_id = 'GM-' . str_pad($max_id, 3, "0", STR_PAD_LEFT);			
		}else{
			$max_id = 'GM-001';
		}
