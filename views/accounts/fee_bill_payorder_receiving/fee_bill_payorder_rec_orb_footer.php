...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            <?php if($this->data['can_user_view'] == 1 && $this->data['can_user_delete'] == 0 && $this->uri->uri_string() == 'accounts/fee_bill_upload_mis') {?>
            if ($('#fee_bill_mis_errors_table').length) {
                var oTable = $('#fee_bill_mis_errors_table').dataTable({
                    "oLanguage": {
        ...
        if ($('#date, #payorder_date').length) {
            $("#date, #payorder_date").mask('9999-99-99', {
                placeholder: 'X'
            });
        }

        if ($('#fee_bill_no').length) {
            $('#fee_bill_no').mask('99-99-99999-9', {
                placeholder: 'X'
            });
        }
...
$(function() {
    ...
    $( "#payorder_date" ).datepicker( "option", "showAnim", "blind");
    $( "#payorder_date" ).datepicker( "option", "dateFormat", "yy-mm-dd");
});
</script>
...
$(document).ready(function(){
    ...
    $("#fee_bill_no").change(function(){
        var GBID = $("#fee_bill_no").val();
 
        if(GBID.length >= 12){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url() . '/ajax/fee_bill_info/getFeeBillInfo';?>",
                data: {
                    gbid: GBID
                },
                success: function(data){
                    $('#feebill_info').html(data);
                },
                error: function() {
