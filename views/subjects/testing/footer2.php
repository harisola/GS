...
$(document).ready(function(){
   ...
   if ($('.dd-dragel').offset().top > x) {
        //Down
       /* $('html, body').animate({
            scrollTop: 1200 // adjust number of px to scroll down
        }, 600); */
...
//}
	...
	if ($('#subjectSubjectForm').length) {
		$("#subjectSubjectForm").validate({
			ignore: [],
			rules: {
				sname: { required: true},
				sdname: { required: true},
			},
			messages: {
				sname: {
				...
				sdname: {
					required: 'Please enter subject display name'
				},
			},
			 // Ajax form submition
                submitHandler: function (form) {
                    $(form).ajaxSubmit({
						type:"POST",
						url : "<?=base_url(); ?>index.php/subjects/ajax_menipulations/createSubject",
						
                        beforeSend: function () {
                            $('#subjectSubjectForm button[type="submit"]').addClass('button-uploading').attr('disabled', true);
                        },
                        uploadProgress: function (event, position, total, percentComplete) {
                            $("#subjectSubjectForm .progress").text(percentComplete + '%');
                        },
                        success: function ( res) {
							...
							//if( response == 1 ){
								$.ajax({
									type: "POST",
									url : "<?=base_url(); ?>index.php/subjects/ajax_menipulations/get",
									dataType: "HTML",
									success: function( respns ){
										$('#showList').html(respns);
									}//
								});
							/*}else if(response==2){
								alert("Inserted, BUT not updated!");
							}else if(response==3){
								alert("Nor Inserted neither updated");
							}*/
							
                          //$("#subjectSubjectForm").addClass('submited');
                          $('#subjectSubjectForm button[type="submit"]').removeClass('button-uploading').attr('disabled', false);
                        },
						complete:function(){
							$( '#subjectSubjectForm' ).each(function(){ this.reset(); });
							$("#subjectSubjectForm .progress").hide();
							$("#subjectSubjectForm").removeClass('submited');
							$("#subjectSubjectForm").addClass('orb-form');
	...
	$(document).on("click","#btn_editable",function(){
		//alert("yes");
		//$("#subEditable")
		
		$("#subEditable").validate({
			ignore: [],
			rules: {
				sname: { required: true},
				sdname: { required: true},
			...
			messages: {
			 ...
			 // Ajax form submition
                submitHandler: function (form) {
                   ...
                   $(form).ajaxSubmit({
						type:"POST",
						url : "<?=base_url(); ?>index.php/subjects/ajax_menipulations/updateSub",
						dataType: "HTML",
                        beforeSend: function () {
                            $('#subEditable button[type="submit"]').addClass('button-uploading').attr('disabled', true);
                        },
                        uploadProgress: function (event, position, total, percentComplete) {
							$("#subEditable .progress").text(percentComplete + '%');
                        },
                        success: function ( res ) {
							//var response = parseInt(res);
							//$( '#subEditable' ).each(function(){ this.reset(); });
							$('#showList').html(res);
							$( '#sname' ).val('');
							$( '#sdname' ).val('');
							$( '#scret_ID' ).val('');
							//$( '#btn_editable' ).attr('disabled', true);
							$('#subEditable button[type="submit"]').removeClass('button-uploading').attr('disabled', false);
							$('#subEditable button[type="submit"]').html('Create');
							$("#subEditable").attr("id","subjectSubjectForm");
							$("#subEditable").attr("name","subjectSubjectForm");
...
$(document).ready(function(){
    $('[name=new_subject_reset_button]').click(function()  {        
        $('[name=subject_name]').val("");
        $('[name=subject_dname]').val("");        
        $('#subject_name').focus();
    });
});
</script>
