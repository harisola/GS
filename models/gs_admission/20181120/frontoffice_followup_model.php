...
WHERE fb.student_id <> 7060 AND fbr.id IS NOT NULL AND af.form_status_stage_id != 7 AND af.form_status_stage_id != 6 AND af.form_status_stage_id != 15 AND af.offer_date != '0000-00-00' AND afo.signed_offer_letter = 1 AND (afo.completed_fif_form = 0 OR afo.signed_hand_book = 0)";
			$result = $this->ddb->query( $query );
			if( $result->num_rows() > 0 ){
				$results = $result->result_array();
				return $results;
