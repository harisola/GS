...
class Visitor_applicant extends MY_Controller {
	...
	public function index()		
	{
		...
		$this->applicant_nic = "";
		$this->applicant_name = "";
		$this->applicant_mobile_phone = "";
		$this->applicant_visit_purpose = "";
		$this->applicant_visit_to_person = "";
		$this->applicant_visit_to_dapartment = "";
		$this->applicant_assigned_card = "";


		if($this->form_validation->run() == FALSE){
			if(count($_POST))
			{
				$this->applicant_nic = $this->input->post('cnic');
				$this->applicant_name = $this->input->post('name');
				$this->applicant_mobile_phone = $this->input->post('mobile_phone');
				$this->applicant_visit_purpose = $this->input->post('applicant_visit_purpose');
				$this->applicant_visit_to_person = $this->input->post('applicant_visit_to_person');
				$this->applicant_visit_to_dapartment = $this->input->post('applicant_visit_to_dapartment');
				$this->applicant_assigned_card = $this->input->post('applicant_assigned_card');
			}

			$this->load->view('threshold/applicant/visitor_applicant_assigncard');
			$this->load->view('threshold/applicant/visitor_applicant_footer_orb');	
	...
	public function add()
	{
		if(count($_POST))
		{
			...
			$this->visitor_applicant_model->save($data);
			redirect('/threshold/visitor_applicant');
		}
	}
}