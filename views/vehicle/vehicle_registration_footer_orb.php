...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            <?php if($this->data['can_user_view'] == 1 && $this->data['can_user_delete'] == 0 && $this->uri->uri_string() == 'vehicle/new_vehicle') {?>
			
			
			
            //if ($('#vehicle_change_form_table').length) {
            //var oTable = $('#vehicle_change_form_table').dataTable({
...
if( league != '' ){
	$.ajax({ 
		type: "POST",
		url: "<?=site_url().'/vehicle/new_vehicle_ajax/getLeague';?>",
		cache: false,
		data: { league: league },
		dataType : "HTML",
		success: function( res ){
			$('#team').find('option').remove().end().append(res);
		},
	});
}
});
...
$(document).ready(function(){
	$('[name=visitor_worker_assignedcard_reset_button]').click(function()  {        
		restform();
     });
});

function restform(){
	$('[name=actionType]').val(1);
	$('[name=updatedID]').val(0);
	$('[name=registered]').val("");
	$('[name=register_type]').val("");
    $('[name=gvid]').val("");
    $('[name=namecode]').val("");
	$('[name=namecodehidden]').val("");
	...
	$('[name=rfdec]').val("");
	$('[name=rfhex]').val("");
	$("#gvid").prop("disabled", false); 
	//$("#regno").prop("disabled", false); 
	//$("#rfhex").prop("disabled", false);
	$('#registered').focus();
}
...
$("#review-form").validate({
	...
	rules: {
		registered: { required: true, valueNotEquals: "default" },
		gvid: { required: true },
		namecode: { required: true },
		namecodehidden: { required: true },
		employee_id: { required: true },
		regno: { required: true },
		league: { required: true, valueNotEquals: "default" },
		team: { required: true , valueNotEquals: "default"},
		colour: { required: true, valueNotEquals: "default" },
		rfdec: { required: true },
		rfhex: { required: true }
	},
	// Messages for form validation
	messages: {
		registered: { required: 'Please enter register type', valueNotEquals: "Please select an item!" },
		 
		gvid: { required: 'Please enter gv id' },
		namecode: { required: 'Please enter name code' },
		namecodehidden: { required: 'Please reselect staff' },
		employee_id: { required: 'Please reselect staff' },
		regno: { required: 'Please enter registration number' },
		league: { required: 'Please enter brand name', valueNotEquals: "Please select an item!" },
		team: { required: 'Please enter make by', valueNotEquals: "Please select an item!" },
		colour: { required: 'Please enter color', valueNotEquals: "Please select an item!" },
		rfdec: { required: 'Please enter rfdec' },
		rfhex: { required: 'Please enter rfdec' },
	},
	// Ajax form submition					
	submitHandler: function (form) {
		$(form).ajaxSubmit({
			...
			success: function (res) {
				...
				$.ajax({ 
					type: "POST",
					url: "<?=site_url().'/vehicle/new_vehicle_ajax/getRightSide';?>",
					cache: false,
					dataType : "HTML",
					success: function( h ){
						$("#rightSideData").html('');
						$("#rightSideData").html(h);
					},
				});
...
// Registering type Employee type staff Almakkaha temproray
$(document).on("change","#registered_type", function(){
	var type = $("#registered_type").val();
	var staffData = $('#staffData');
	var name_code1 = $('#name_code1');
	var name_code2 = $('#name_code2');
	if( parseInt( type ) == 1 ){
		name_code1.hide();
		name_code2.show();
	}else{
 ...
 $(document).on("select2-removed", "#staffData", function(e) {
		console.log("removed val=" + e.val + " choice=" + e.choice.text);
	});


// Staff Dropdown		
$(document).on("select2-highlight", "#staffData", function(e) {
	//console.log("highlighted val=" + e.val + " choice=" + e.choice.text);
	$("#namecodehidden").val('');
	$("#namecodehidden").val(e.choice.text);
	$("#namecode").val('');
	$("#employee_id").val(e.val);
});
		
		

// right side anchor clicked
$(document).on("click", ".vehicalID", function(){
	var vehicalID = $(this).attr("data-id");
	
	
	
	if(  parseInt( vehicalID ) > 0 ){
		
		$.ajax({ 
		type: "POST",
		url: "<?=site_url().'/vehicle/new_vehicle_ajax/getLeagueById';?>",
		cache: false,
		data: { league: vehicalID },
		dataType : "JSON",
		success: function( r ){
			$("#actionType").val(2);
			$("#updatedID").val(vehicalID);
			$("#registered_type").val();
			$("#gvid").val( r.gv_id );
			$("#gvid").prop("disabled", true); 
			$("#regno").val( r.registration_no );
			...
			$("#rfdec").val( r.rfid_dec_no );
			$("#rfhex").val( r.rfid_hex_no );
			//$("#rfhex").prop("disabled", true); 
			$('#registered_type option').each(function() {
				if($(this).val() == r.register_type ) {
					$(this).prop("selected", true);
				}
			});
			$('#color_id option').each(function() {
				if($(this).val() == r.color_id ) {
					$(this).prop("selected", true);
				}
			});
			if( r.brand_id > 0 ){
				$('#league option').each(function() {
					if($(this).val() == r.brand_id ) {
						$(this).prop("selected", true);
					}
				});		
			}//
		if( r.employee_id > 0 ){
			$('#staffData option').each(function() { 
				if($(this).val() == r.employee_id ) {
					$(this).prop("selected", true);
				}
				$("#select2-chosen-1").html(r.name_code);
			});
		}//
		
		if(r.register_type == 1 ){
			$('#name_code1').hide();
			$('#name_code2').show();
			
			$("#namecode").val(r.name_code);
			$("#namecodehidden").val( r.name_code );
