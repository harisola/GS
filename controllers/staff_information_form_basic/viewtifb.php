...
class Viewtifb extends MY_Controller {
	...
	public function index(){
		...
		if($this->input->post('solangigtID') != ''){
			...
			$this->load->model('tif_model/tif_form_model');
			 ...
			 if( $this->tif_form_model->getStaffInfo($solangigtID) ){
				$this->makePDFFile($solangigtID);
			 }else{
				 $data["noRecordFound"] = "No Record Found";
				 //$this->load->view('staff_information_form/fif_form_id2',$data);
		...
		if($this->form_validation->run() == FALSE){
			...
			$this->load->view('staff_information_form/fif_form_id2');
			$this->load->view('staff_information_form/footer.php');
