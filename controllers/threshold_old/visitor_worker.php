...
class Visitor_worker extends MY_Controller {
	...
	public function index()		
	{
		...
		$this->worker_nic = "";
		$this->worker_name = "";
		$this->worker_mobile_phone = "";
		$this->worker_visit_purpose = "";
		$this->worker_visit_to_person = "";
		$this->worker_visit_to_dapartment = "";
		$this->worker_assigned_card = "";


		if($this->form_validation->run() == FALSE){
			if(count($_POST))
			{
				$this->worker_nic = $this->input->post('cnic');
				$this->worker_name = $this->input->post('name');
				$this->worker_mobile_phone = $this->input->post('mobile_phone');
				$this->worker_visit_purpose = $this->input->post('worker_visit_purpose');
				$this->worker_visit_to_person = $this->input->post('worker_visit_to_person');
				$this->worker_visit_to_dapartment = $this->input->post('worker_visit_to_dapartment');
				$this->worker_assigned_card = $this->input->post('worker_assigned_card');
			}

			$this->load->view('threshold/worker/visitor_worker_assigncard');
			$this->load->view('threshold/worker/visitor_worker_footer_orb');	
	...
	public function add()
	{
		if(count($_POST))
		{
			...
			$this->visitor_worker_model->save($data);
			redirect('/threshold/visitor_worker');
		}
	}
}