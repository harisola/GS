...
$(document).on("change", ".domain", function(){
	...
	if( ID == 2  ){  }else{ $("#gs_grade").hide(); $("#gs_subject").hide(); }
	...
	//}else{alert("Position Created Refresh The Page");}	
});
...
$(document).on("change", ".category", function(){
	...
	var $row = $(this).closest(".containerTable");
	//$row.find(".category").val('');
	var gs_domain = $("#gs_domain").val();
if( ID != '' ){
	if(ID == '9_K' && gs_domain==2  ){ $("#gs_grade").show(); $("#gs_subject").show(); }else{ $("#gs_grade").hide(); $("#gs_subject").hide(); }
	var idS = ID.split("_");
	var nText = idS[1];
	var txt = $(this).find('option:selected').text();
...
$(document).on("click",".lsclink", function(){
	...
	var pStatus = $row.find(".positionStatus");
	pStatus.val(1);
	
	var tableOne = $row.find(".tableOne");
	
	var gs_grade;
	var gs_subject;
	if ($('#gs_grade').is(":visible")){
		gs_grade = $("#gs_grade").val();
		gs_subject = $("#gs_subject").val();
	}else{
		gs_grade =0;
		gs_subject=0;
	}
	...
	if( posID != '' ){
		$.ajax({
			...
			data:{posID2:posID,domain2:domainID,roletype2:roleID,category2:catID,gs_grade:gs_grade,gs_subject:gs_subject},
			dataType: "JSON",
			success:function(res){
				//alert(res.lastID);
				positionIDLabel.html( res.gpID  );
				...
				lastID.val(pID);
				fundID.val("I_"+pID);
				tableOne.removeClass("firstActive");
				positionIDT.addClass("firstActive");
			},
		});
	}
...
$(document).on("input", ".bottomline", function(){
	...
	var tableThree = $row.find(".tableThree");
				...
				if( res.affected > 0 ){
					...
					$.ajax({
						...
						success:function(res){
							...
							positionIDT.removeClass("firstActive");
							tableThree.addClass("firstActive");
...
$(document).on("change",".rpRoleCode2",function(){
	...
	//var rpRoleCode2 = $row.find(".rpRoleCode2");
	var rpRoleCode2 = $row.find(".positionID2");
	if( parseInt( thsID ) > 0 ){
		$.ajax({
			type:"POST",
			url : "<?=base_url(); ?>index.php/roles/position_ajax/updttpbl",
			...
			success:function(res){
				$.ajax({
					...
					success:function(resp){
						...
						rpRoleCode2.find('option').remove().end().append( resp.options2 );
					},
				});
...
$(document).on("change",".link_1",function(){
	...
	var tableFour = $row.find(".tableFour");
	tableFour.addClass("firstActive");
});
	...
	$.ajax({
		...
		success:function(res){
			...
			$.ajax({
				...
				cache: false,
				data:{staff:ID,lastID:lastID,ptype:1},
				dataType: "JSON",
				success:function(resp){
...
$(document).on("change", ".cat2", function(){
	...
	var fundamental2 = $row.find(".fundamental");
	var locationSerial = $row.find(".locationSerial");
	...
	var pRSName = $row.find(".pRSName");
	var pRSName2 = $row.find(".pRSName2");
	var fCount = $row.find(".fundaCount").val();
	...
	var tableThree = $row.find(".tableThree");
	// A means Previos.
	// B means this position.
	var fundID = $row.find(".fundamentalID").val();
	var ctRC = $row.find(".countRoleCode");
	var ctRC1 = ctRC.val();
	var nameCode3 =someVariable.trim();
	if( nameCode3 == 'Role Code' ){ nameCode3=posID; }
	if( parseInt( pStatus ) != 0 ){
	$.ajax({
		type:"POST",
		url : "<?=base_url();?>index.php/roles/position_ajax/finish",
		cache: false,
		data:{staff1:staffID,posID1:posID,thsID1:thsID,rolecode1:nameCode3,rolecode2:r4,countRC:ctRC1,funda:fundamental,funda2AB:fundaAB,fundID2:fundID},
		dataType: "JSON",
		success:function(resp){
			pStatus2.val(0);
			...
			$row.find('input:text').val('');
			 ...
			 roleCode2.html('');
			 roleCode1.html("Role Code");
			 gt_ID.html("GT-ID");
			 nameCode.html("Staff Name");
			 
			 pRSName.html("Staff Name");
			 pRSName2.html("Staff Name");
			 
			 
			 staffCount.html("Count");
			 fundamental2.html("");
			 locationSerial.html("Location Serial");
			 positionIDLabel2.html("Position ID");
			 posTitleTB.html("Position Title");
			 posTitleTB2.html("Position Title");
			 ctRC.val(1);
			 
			 tableFour.removeClass("firstActive");
			 tableThree.removeClass("firstActive");
			 tableOne.addClass("firstActive");
			 
			 alert("Position Created !");
		},
	});
	}else{ alert("Please create Position !"); }
