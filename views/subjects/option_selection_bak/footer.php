	...
	if( gradeID != '' ){
		$.ajax({
			...
			success: function( response ){
				//console.log(response.grdStdnt);
				//$("#selectedGrade").html(response);
...
var previous;
$(document).on("focus",".groupID", function(){
previous = $(this).val(); 
});
...
var thisValueP = parseInt( thisValue );
...
var grdgrp112 = parseInt( grdgrp1_12 );

var grdGrpPreId = "grdgrp_"+_grp+"_"+previous;
var grdgrp1Pre = $("#"+grdGrpPreId).text();
var grdgrpPre112 = parseInt( grdgrp1Pre );

if( previous == ''  ){
	var old = grdgrpPre112;
}else{
	var old = ( grdgrpPre112 - 1 );	
}

$("#"+grdGrpPreId).text(old);

 if( thisValue != '' || thisValue != 'NaN' ){
	var grdGrpNewTxt = ( grdgrp112 + 1 );
}else{
	var grdGrpNewTxt = grdgrp112;
}
...
var grpSbID = $(this).val();
	...
	if(studentIDSize != '' ){
		studentID.each(function(i){
			var thisValue = $(this).val();
			//console.log("Students ID "+thisValue);
			tSID.push(thisValue);
			//$('#sIDAr').append('<li id="'+thisValue+'">"'+i+'"</li>');
		});
	}
	// to  here ***/ 
	
	//$("#studentsIDAr").val(tSID);
	//console.log( tSID );
