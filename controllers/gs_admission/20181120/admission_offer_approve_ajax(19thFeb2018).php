...
class Admission_offer_approve_ajax extends CI_Controller{
	...
	public function get_offer_detail(){
		...
		if($offer->is_signed_offer_letter == 1){
			...
			$html .= '<li id="signed_offer_'.$offer->form_id.'" class="cross">Offer Lette Signedr</li>';
		...
		$html .= '<li '.$offer_submission_style.' class="offer-submission-hide"><a href="javascript:void(0)" data-toggle="modal" data-target="#OfferSubmission" data-formid = "'.$offer->form_id.'" id="offer_submission">Offer Submission</a></li>';
	...
	public function add_detail(){
		...
		if(!empty($concession_detail)){
			...
			$html .= '<input type="date" placeholder="dd/mm/yyyy" name="date_of_payment" value="'.$concession_detail[0]->due_date.'"  required/>';
			}else{
	...
	public function insert_address_concession(){
		...
		if($this->input->post('previous_grade')){
			$previous_grade = $this->input->post('previous_grade');
		}else{
			$previous_grade = null;
		}

		if($this->input->post('previous_school')){
	...
	public function add_preperation(){
		...
		$html .= '<label class="bigLabel" for="5"> Print Fee bill <a target="_blank" href="'.base_url().'index.php/gs_admission/admission_fee_bill/fb?FormID='.$form_id.'" class="smallLink"><em>Print</em></a></label>';
