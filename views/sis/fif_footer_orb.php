...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // Student Information
        $('.student_official_name a').editable({
            type: 'text',
            name: 'official_name',
            title: 'Student Official Name',
...
if ($('.guardianCNICIN').length) {
    $('.guardianCNICIN').mask('99999-9999999-9', {
        placeholder: 'X'
    });
}

// var currentradio= $("input[name='ATM']:checked")[0];
...
//$("input:radio[name^='ATM']").change(function(){
var clicks = new Array();
var Array_Student_id = new Array();
    ...
    $("input:radio").each( 
        function()
        {
             ...
             if ($(this).is(":checked")) 
             {
                ...
                var ss = _val.split("_");
                var fif_gfid = ss[1];
                Array_Student_id.push(fif_gfid) 
                clicks.push(_val) 
...
$(".Advance_tax").change(function(){

        var _val = $(this).val();

       // var fif_gfid = $('input[name="fif_gfid"]').val();
        ...
        var n = fif_gfid+'_ATM'; 
        var currentradio= $("input[name^='"+n+"']:checked")[0];
       

        var Father_ch = "Father_"+fif_gfid;
        var Mother_ch = "Mother_"+fif_gfid;
        var Guardian_ch = "Guardian_"+fif_gfid;        



        //var newradio= $("input[name='ATM']:checked")[0];

        if( Father == 'Father')
        {
            _NIC = $("#parent_father_nic").text();
        }else if( Father == 'Mother'){
            _NIC = $("#parent_mother_nic").text();
        }else{
            _NIC = '';
        }
        ...
        bootbox.dialog({
        message: "Are you sure you want to change tax nic?",
...
$("#Father_late").change(function() { 
    var Type='Father_late';
    var operation = 0;
    if($(this).prop('checked') == true) {
      operation = 1;
      parent_late(Type,operation);
    } else {
      parent_late(Type,operation);
    }
});

$("#Mother_late").change(function() { 
    var Type='Mother_late';
    var operation = 0;
    if($(this).prop('checked') == true) {
      operation = 1;
      parent_late(Type,operation);
...
function parent_late(Type, operation)
{
    var fif_gfid = $('input[name="fif_gfid"]').val();
    bootbox.dialog({
        message: "Are you sure you want change this?",
        title: "Please Confirm.",
        buttons: {
            confirm: {
                label: "Yes!",
                callback: function() { 
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url()?>/sis/ajax_update/Method_update_family_record",
                        data:{ Type:Type,operation:operation,fif_gfid:fif_gfid },
                        dataType:"JSON",
                        cache: false,
...
$(".guardianCNICINBtn").click(function(){
   // var fif_gfid = $('input[name="fif_gfid"]').val();
    var fif_gfid  = $(this).data("save_id");

    

    var guardianCNICIN = $("#guardianCNICIN_"+fif_gfid).val();
    ...
    if(guardianCNICIN != '' )
    {
        ...
        callback: function() { 
          //  currentradio= newradio;
            ReplaceNIC( guardianCNICIN, fif_gfid );
        } //// end callback: 
        },
        cancel: {
        label: "Cancel",
...
function ReplaceNIC( NIC_Num, fif_gfid )
{
    $.ajax({
         type: "POST",
         url: "<?php echo site_url()?>/sis/ajax_update/replace_index",
         data:{ NIC_Num:NIC_Num,fif_gfid:fif_gfid },
         dataType:"JSON",
         cache: false,
         beforeSend: function() { /*$("#ajaxloader").show();*/ },
         success: function(res){
