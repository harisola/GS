...
//$('#ParentContainer').scroll(function() {  $('#FixedDiv').css('top', $(this).scrollTop()); });
...
//$(document).on("scroll", "#ParentContainer", function(){ 	console.log("Scrolling"); });
//	========================================================================
//	Datatables
// 	========================================================================
$(document).on("change","#gradeID",function(){
	...
	if( gradeID != '' ){
		...
		$.ajax({
			...
			data: { jsonstring:gradeID,sessionID:sessionID },
			dataType: "JSON",
...
$(document).on("click", "#createBlock", function(){
	...
	var block 	  = parseInt( $("#groupBlockNo").val() );
	var gradesID  = parseInt( $("#gradesID").val() );
	...
	if( block > 0 && block < 6 ){
		...
		$.ajax({
			type: "POST",
			url : "<?=base_url(); ?>index.php/subjects/ajax_opt_level",
			data: { block:block,gradesID:gradesID,groupID:groupID,subjectID:subjectID,sessionID:sessionID,term:term,totalStudents:totalStudents },
			dataType: "JSON",
			success: function( res ){
				$("#loadingStudentsAjax2").hide();
				//console.log( res );
				$("#FixedDiv").find("tr:gt(0)").remove();
				$("#FixedDiv").append( res.blockHtml );
				
				//$("#subjectStudents").find("tr:gt(0)").remove();
				$("#subjectStudents  > tbody > tr").remove();
				$("#subjectStudents").append( res.subStudentsList );
				//alert( res.subStudentsList );
			}
...
//$('.blockDropdown').data('pre', $(this).val());

$(document).on("change", ".blockDropdown", function(){
	//var before_change = $(this).data('pre');//get the pre data
	var D = $(this).attr("id");
	var blockID = $(this).val();
	var i = D.split("-");
	//grd_1- std_6035 - grp_2
	var _std = i[1];
	var std = _std.split("_");
	var stdID = parseInt( std[1] );
	//alert(D);
	var  _findID;
	var  _findText;
	_findID = 0;
	...
	$('#listID li').each(function(){
		var _thisValue = parseInt( $(this).val() );
		if( _thisValue == stdID) {
			_findID = _thisValue;
			previousBlock = parseInt( $(this).text() );
	...
	var gndr = $("#gndr_"+stdID).val();
	var bID = $("input[name='blockID']").val();
	//var bID = $("input[name='gradesID']").val();
	...
	var b1 =  $("#boy_"+previousBlock);
	var b2 = b1.text();
	var b3 = parseInt(b2);
	
	
	var b11 =  $("#boy_"+blockID);
	var b22 = b11.text();
	var b33 = parseInt(b22);
		
	var g1 =  $("#girl_"+previousBlock);
	var g2 = g1.text();
	var g3 = parseInt(g2);
	
	var g11 =  $("#girl_"+blockID);
	var g22 = g11.text();
	var g33 = parseInt(g22);
	...
	var preTotal1 =  $("#total_"+previousBlock);
	var preTotal2 = preTotal1.text();
	var preTotal3 = parseInt( preTotal2 );
	
	
	
	
	var curTotal1 =  $("#total_"+blockID);
	var curTotal2 =  curTotal1.text();
	var curTotal3 = parseInt( curTotal2 );
	
	if( preTotal3 > 0 ){
		
		var preTotal4 = ( preTotal3 - 1 );
		preTotal1.html( preTotal4 );
		
		var curTotal4 = ( curTotal3 + 1 );
		curTotal1.html( curTotal4 );
	...
	$('#listID').append('<li value="'+stdID+'">'+blockID+'</li>');	
	
	
		$.ajax({
			type: "POST",
			url : "<?=base_url(); ?>index.php/subjects/ajax_opt_level/stdSltdblck",
			data: { bID:bID,stdID:stdID,blockID:blockID },
			dataType: "JSON",
			success: function( res ){
				console.log(res);
			}
