...
(function ($) {
    ...
    $(document).ready(function () {
	   ...
	   $(document).on("change","#teacherGrade", function(){
		  var ID = parseInt( $(this).val() );
		  if( ID > 0 ){
			$.ajax({
				type :"POST",
				url  : "<?=base_url(); ?>index.php/etab/ajax/getGrdSb",
				data :{ grdID:ID },
				cache:false,
				dataType:"JSON",
				success:function(res){
					$("#subjectList").html(res.htmlS)
				},
			});
		  }
	   });
	   ...
	   $(document).on("change","#gSID", function(){
		  var ID = parseInt( $(this).val() );
		  var grdID = parseInt( $("#teacherGrade").val() );
		  
		  
		  if( ID > 0 ){
			$.ajax({
				type :"POST",
				url  : "<?=base_url(); ?>index.php/etab/ajax/getGrdSbAss",
				data :{ sID:ID,grdID:grdID },
				cache:false,
				dataType:"JSON",
				success:function(res){
					$("#assCat").html(res.assCat)
				},
			});
		  }
	   });
	   ...
	   // ==== take grade id and get section ====
			$(document).on("change","#gradeName",function(){
				...
				$.ajax({
					...
					url:"<?php echo base_url(); ?>index.php/etab/ajax/grdSubjects3",
					cache:false,
					data:{grade_id:gradeID,userID:userID},
					dataType:'JSON',
					success:function( res ){
						//alert( res.HTML2 );
			...
			$.ajax({
					type:"POST",
					url:"<?php echo base_url(); ?>index.php/etab/ajax/getGradeSection",
					cache:false,
					data:{grade_id:gradeID,userID:userID,sessionID:sessionID},
					dataType:'HTML',
					success:function( res ){
						//console.log( res );
						$('#grdSections').html( res );
			...
			$(document).on("change","#gradeSubject", function(){
				...
				if( gradeID > 0 && subjectID > 0 ){
					...
					$.ajax({
						type:"POST",
						url:"<?php echo base_url(); ?>index.php/etab/ajax/gradeCategoryFinal",
						cache:false,
						data:{grade_id:gradeID,subjectID:subjectID},
						dataType:'JSON',
						//dataType:'HTML',
						success:function( res ){
							$('#assCat').html( res.HTML2 );
						}
					});
		...
		$(document).on("change","#mainCategory", function(){
				var catID = $(this).val();
				var gradeID = $("#gradeName").val();
				var subjectID = $("#gradeSubject").val();
				if( gradeID > 0 && subjectID > 0 ){
					
					// get grade Categories and its subcategories
					$.ajax({
						type:"POST",
						url:"<?php echo base_url(); ?>index.php/etab/ajax/gradeSubCategoryFinal",
						cache:false,
						data:{grade_id:gradeID,subject_id:subjectID,categoryID:catID},
						dataType:'JSON',
						//dataType:'HTML',
						success:function( res ){
							$('#assCatType').html( res.HTML3 );
						}
					});
				}
			});
