...
class Interaction_form_send_modal extends CI_Controller {
	...
	{	
      ...
      // Interaction Center
      $modal_line_interaction_center .= '
      <form method="post" action="' . site_url() . '/hcm/interaction_hr/add2" class="orb-form">
      ...
      foreach($interaction_centre as $center) {
        if($recommended_interaction_centre[0]->forward_interaction_id == $center->id){
          $modal_line_interaction_center .= '<option value="' . $center->id . '" selected="selected">' . $center->name . '</option>';
        }else{
