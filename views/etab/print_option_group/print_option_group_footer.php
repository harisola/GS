...
<div id="ReviveCase" class="modal-dialog" title="Are you sure?" style="display:none">
...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $('.left-toggler').click(function (e) {            
            ...
            $(".head-wrapper").toggleClass("main-head-toggle-left");                        
            e.preventDefault();
        });
...
<script type="text/javascript" src="<?php echo base_url()?>components/orb/js/vendors/js-xlsx/xlsx.core.js"></script>
<script type="text/javascript" src="<?php echo base_url()?>components/orb/js/vendors/FileSaver/FileSaver.js"></script>
<script type="text/javascript" src="<?php echo base_url()?>components/orb/js/vendors/TableExport/src/stable/js/tableexport.js"></script>
...
// $(document).ready(function() {
//     var studenData =  $('#studentlist2').DataTable({
//         dom: 'Bfrtip',
//         buttons: [
...
$(document).on("change","#gradeID",function(){
	...
	if( gradeID != '' ){
		$.ajax({
			...
			url : "<?=base_url(); ?>index.php/etab/print_option_group_ajax/get_option",
			data: { grade_id:gradeID,academic_session_id:sessionID,term:term },
			dataType: "JSON",
			success: function( response ){

                $('#studentlist2').show();
                $('#studentlist2 thead').html('');
                $("#studentlist2  tbody").html('');
                var class_list  = response.classList;
                var program = response.program;
                var group = response.group;
                
                              
                var option_group = response.option_group;

                var head = '';
        
                head += '<tr>';
                head += '<th>#</th><th>GS-ID</th><th>Abridge Name</th><th>Gender</th><th>Section</th><th>Status</th>';


                // GROUP NAME
                for(var i = 0 ; i <group.length;i++){
                    head += '<th class="text-center">'+group[i].group_name+'</th>';
                }

                head += '</tr>';

                $("#studentlist2  thead").append(head);

                var tbody = '';

                for(var i = 0 ; i < class_list.length ; i++){
                    tbody += '<tr>';
                    tbody += '<td class="text-center">'+parseInt(i+1)+'</td>';
                    tbody += '<td class="text-center">'+class_list[i].gs_id+'</td>';
                    tbody += '<td class="text-center">'+class_list[i].abridged_name+'</td>';
                    tbody += '<td class="text-center">'+gender_name(class_list[i].gender)+'</td>';
                    tbody += '<td class="text-center">'+class_list[i].section_name+'</td>';
                    tbody += '<td class="text-center">'+class_list[i].std_status_code+'</td>';
                    for(var j = 0 ; j < group.length ; j++){
                        var found = 0;
                        for(var k = 0 ; k < option_group.length ; k++){
                            if(class_list[i].id == option_group[k].student_id && group[j].group_id == option_group[k].group_id && found == 0 ){
                                    found = 1;
                                    tbody += '<td class="text-center">'+option_group[k].subject_name+'</td>';
                            }
                        }
                    }
                 ...
                 $("#studentlist2  tbody").append(tbody);

                 $('.btn-toolbar').html('');



                $("#studentlist2").tableExport();
...
var grpSbID = $(this).val();
	...
	data["_previous"] = parseInt( _previous );
