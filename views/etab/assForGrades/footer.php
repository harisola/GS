...
<div id="myModal5" class="modal fade" data-easein="fadeInUp" data-easeout="fadeOutUp" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
<div class="modal-dialog modal-sm">
<div class="modal-content">
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<div class="modal-body">
<p>This Category Type Weightage Can Not Be Zero</p>
</div>
<div class="modal-footer">
<button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Close</button>
</div>
	...
	*/
		...
		// ==== take grade id and get section ====
			...
			$(document).on("change",".weighateges",function(){
				...
				var blockIDs = $('.blockIDs').eq(cur).val();
				var blockID = parseInt( blockIDs );
				var cvalue = parseFloat( $(this).val() );
				
				//console.log( blockID );
				//console.log( cvalue );
				if( blockID == 1 &&  cvalue == 0 ){ 
...
$(document).on("click", "#formativeType", function(){
	...
	if( ( session != '' && session >  0 ) && ( term != '' && term > 0 ) && ( grade != '' && grade > 0 ) && (subject!='' && subject>0) && (category != '' && category>0) ){
		$("#assessmentGrade").html('');
		$.ajax({
			type: "POST",
			url : "<?=base_url();?>index.php/automcomplete/assGrade/editAssessment",
			data: {session:session,term:term,grade:grade,subject:subject,category:category},
			dataType: "HTML",
			 beforeSend: function(){
				$('#loadingImage').show();
			},
...
$(document).on("click", "#summativeType", function(){
	var session = $("#session").val();
	var term = $("#term").val();
	var grade = $("#grade").val();
	var subject = $("#subject").val();
...
$(document).on("click",".updateAssessmentBtn", function(){
	...
	if ($('#createAssForm').length) {
		   ...
		   $("#createAssForm").validate({
                ...
                submitHandler: function (form) {
                    $(form).ajaxSubmit({
						type: "POST",
						url : "<?=base_url();?>index.php/automcomplete/assGrade/overWriteAssessment",
						cache: false,               
						data: $('#createAssForm').serialize(),
						dataType: "TEXT",
						beforeSend: function () {
                        ...
                        success: function ( reponseCrAs ) {
							$.ajax({
								type: "POST",
								url : "<?=base_url();?>index.php/automcomplete/assGrade/getAssessment",
								data:{as:session_id,ast:term_id,grdn:grade_id,grdSub:subject_id,mcat:category_id},
								dataType: "html",
								success: function( responseGtAss ){
									//alert(responseGtAss);
									$('#createdCategories2').html( responseGtAss );
