...
class Notify_log_admissions extends MY_Controller {
	...
	public function get_notification($userid){
		...
		$category=1;
		$unreads = $this->ABM->get_admission_notification_method( $category, $userid );
		if( !empty($unreads) ){
			$unread = $unreads;
		}else{
			$unread =0;
	...
	public function Attendance_Notification()
	{
		...
		$Query="select af.id as Form_id from atif_gs_admission.admission_form as af
			where (af.form_status_id=2 and af.form_status_stage_id=3)
				
			and ( DATEDIFF(curdate(),af.form_submission_date) = 2 )

			union 
			select af.id as Form_id from atif_gs_admission.admission_form as af
			where )af.form_status_id=3 and af.form_status_stage_id=3)
			and ( DATEDIFF(curdate(),af.form_assessment_date) = 2 )


			union 
			select af.id as Form_id from atif_gs_admission.admission_form as af
			where )af.form_status_id=4 and af.form_status_stage_id=3)
			and ( DATEDIFF(curdate(),af.form_discussion_date) = 2 )


			union 
			select af.id as Form_id from atif_gs_admission.admission_form as af
			where (af.form_status_id=5 and af.form_status_stage_id=3)
