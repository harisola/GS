...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // ========================================================================        
        if ($('#visitor_vendor_assignedcard_form').length) {
            $("#visitor_vendor_assignedcard_form").validate({
        ...
        if ($('#mobile_phone').length) {
		 ...
		 if ($('#mobile_phone2').length) {
            $('#mobile_phone2').mask('0999-9999999', {
                placeholder: 'X'
            });
        }
...
$(document).ready(function(){
    $('[name=visitor_vendor_assignedcard_reset_button]').click(function()  {        
        $('[name=cnic]').val("");
        $('[name=name]').val("");
        $('[name=mobile_phone]').val("");
        $('[name=vendor_visit_purpose]').val("");
        $('[name=vendor_visit_to_person]').val("");
        $('[name=vendor_visit_to_dapartment]').val("");
        $('[name=vendor_assigned_card]').val("");
...
$(document).on("click","#searchQuery", function(){
	...
	$.ajax({
		type:"POST",
		cache:false,
		url: "<?=site_url().'/ajax/searchQuery/search';?>",
		data:{ sQuery:cnic,mobilePhone:mobilePhone },
		dataType:"JSON",
		beforeSend: function(){ 
			$('[name=cnic]').val("");
			$('[name=name]').val("");
		...
		success:function(res){
			...
			if( res.nic ){
				//$('[name=cnic]').val(cnic);
				$('[name=cnic]').val(res.nic);
				$('#vendor_assigned_card').focus();
				$("#cnic").val('');
			}else{
				$('[name=cnic]').val(cnic);
				$('#name').focus();
			...
			if( res.mobile_phone ){
				...
				$('[name=mobile_phone]').val(res.mobile_phone);
				$("#mobile_phone2").val('');
			}else{
				$('[name=mobile_phone]').val(mobilePhone);
				$("#mobile_phone2").val('');
				$('[name=cnic]').focus();
			}
		...
		setTimeout(function(){
			...
			//$('[name=mobile_phone]').val(res.mobile_phone);
			$('[name=vendor_visit_purpose]').val(res.purpose);
			$('[name=vendor_visit_to_person]').val(res.contact_person);
			$('[name=vendor_visit_to_dapartment]').val(res.contact_department);
			$('#vendor_assigned_card').focus();
		}, 1000);	
