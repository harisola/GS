...
<script type="text/javascript">
$(document).on("click", ".assType",function(){
	
	//var $rows = $(this).closest(".panel-body");
	//var rwAsType = $rows.find(".assType");
	//var UID = rwAsType.attr('id');
	
	var d = $(this).attr('id');
	if(d != ''){
		$.ajax({
		type: "POST",
		url: "<?=site_url().'/ajax/subject_head/assigned_marks';?>",
		data: { trID: d},
		dataType : "HTML",
		success: function( res ){
			$("#page_contents").hide();
			$("#page_contents2").show();
			$("#page_contents2").html(res);
		},
...
$(document).on("click",".pedding_request", function(){
	
	var $rows = $(this).closest("tr");
	var hGrade = $rows.find(".hGrade");
	var hGrade = hGrade.val();
	var hSection = $rows.find(".hSection");
	var hSection2 = parseInt( hSection.val() );
	
	
	var dtal_id = $rows.find(".detail_id");
	var detail_id = parseInt( dtal_id.val() );
	var request_type = 0;
	
	if( hGrade != '' && hSection2 > 0 && detail_id > 0  ){
		
	$.ajax({
		type: "POST",
		url: "<?=site_url().'/ajax/station_process/pddgReq_Stds';?>",
		data: { grade_id: hGrade,section_id:hSection2,detail_id:detail_id,request_type:request_type},
		dataType : "JSON",
		beforeSend : function (){
			//console.log("Before Sending!");
			$('#response').show();
			 $('#response').html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span>Loading...</span>');
			//$("#main_contents").html('');
		},
		success: function( res ){
			//console.log(res);
...
$(document).on("click",".total_request", function(){
...
$(document).on("click",".close_setup_4", function(){
	...
	if( user_id > 0 ){
			$.ajax({
				type: "POST",
				url: "<?=site_url().'/ajax/station_process/teacherNotify';?>",
				data: { user_id: user_id},
				dataType : "json",
				beforeSend : function (){
					//console.log("Before Sending!");
					$('#response').show();
					 $('#response').html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span>Updating...</span>');
					$("#main_contents").html('');
				},
				success: function( res ){
					$("#main_contents").html(res.html);
					//console.log(res);
...
$(document).on("click",".accept_obtain",function(){
	
	
	var $rows = $(this).closest("li");
	var students = $rows.find(".students");
	var students_id = students.val();
	
	//alert("studnt "+students_id);
	var dataID = $(this).attr("data-id");
	
	
	var mrksID = $(this).closest("li").attr("data-id");
	var marksID = parseInt( mrksID );
	
	//alert("marksID "+marksID)
	var marksDecision = $rows.find(".marksDecision");
	marksDecision.fadeOut();
	
	var undo = $rows.find(".undoDiv");
	undo.fadeIn();
	...
	var ob1 = $rows.find(".ob1");
	var finalMarks = $rows.find(".finalMarks");
	
	var ob2 = ob1.html();
	
	var FD = $rows.find(".finalDecision");
	FD.val(1);
	
	
	
	var OM = $rows.find(".marks_obtained");
	var ObtainM = OM.val();
	
	var PM = $rows.find(".pendingMarks");
	var PenddingM = PM.val();
	...
	if( marksID > 0 ){
			$.ajax({
				type: "POST",
				url: "<?=site_url().'/etab/ajax/approveMarks';?>",
				data: { marksID: marksID,students_id:students_id,user_id:user_id,user_ip:user_ip,user_agent:user_agent,ObtainM:ObtainM,PenddingM:PenddingM,FDec:FDec},
				dataType : "HTML",
				success: function( res ){
					console.log(res);
...
$(document).on("click",".accept_new",function(){
	...
	var marksID = $(this).closest("li").attr("data-id");
	//alert(marksID);
...
$(document).on("click",".btn_primary",function(){
	...
	var $rows = $(this).closest("li");
	var FD = $rows.find(".finalDecision").val();
	var fDes =0;
	...
	if( marksID > 0 ){
			$.ajax({
				type: "POST",
				url: "<?=site_url().'/etab/ajax/approveMarks';?>",
				data: { marksID: marksID,students_id:students_id,user_id:user_id,user_ip:user_ip,user_agent:user_agent,ObtainM:ObtainM,PenddingM:PenddingM,FDec:fDes},
				dataType : "HTML",
				success: function( res ){
				...
				$rows.fadeTo("slow", 0.01, function(){ //fade
...
$(document).on("click",".btn-minmax", function(e){
  e.preventDefault();
  var $target = $(this).parent().parent().next('.panel-body');
  if ($target.is(':visible')) $('i', $(this)).removeClass('fa fa-chevron-circle-up').addClass('fa fa-chevron-circle-down');
  else $('i', $(this)).removeClass('fa-chevron-circle-down').addClass('fa fa-chevron-circle-up');
...
$(document).on("click",".unlocked", function(){
	...
	var lockedDiv = $rows.find(".lockedDiv");
	var lockedDiv2 = $rows.find(".lockedDiv2");
	
	var undoDiv2 = $rows.find(".undoDiv2");
	...
	lockedDiv2.fadeOut("slow");
	//lockedDiv.fadeIn("slow");
	undoDiv2.fadeIn("slow");
	 
});

$(document).on("click",".undo2", function(){
	var $rows = $(this).closest("tr");
	//var lockedDiv = $rows.find(".lockedDiv");
	var lockedDiv2 = $rows.find(".lockedDiv2");
	var undoDiv2 = $rows.find(".undoDiv2");
	lockedDiv2.fadeIn("slow");
	//lockedDiv.fadeIn("slow");
	undoDiv2.fadeOut("slow");
});

$(document).on("click",".appAssTpy",function(){
	
	var $rows = $(this).closest("tr");
	var lockedDiv = $rows.find(".lockedDiv");
	var lockedDiv2 = $rows.find(".undoDiv2");
	...
	if( ID > 0 ){
		$.ajax({
			type: "POST",
				url: "<?=site_url().'/ajax/station_process/approveAssmntType';?>",
				data: { marksID: ID,user_id:user_id},
				dataType : "json",
				beforeSend : function (){
				},
				success: function( res ){
