...
<script type="text/javascript">
  ...
  $(document).on('click','#add-profile',function(){
    ...
    $.ajax({
      type:"POST",
      cache:false,
      url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/view_add_profile",
      success:function(e){
  ...
  $(document).on('click','#submit_profile',function(e){
    ...
    var ext_frequency;
    var july_start;
    var sat_working_hours;
    var saturday_on;
    var days;

    // Weeks Days
    var week_day = $('input[name="days[]"]').map(function(){
      return this.value;
    }).get();
    ...
    // Time IN
    var time_in = $('input[name="time_in[]"]').map(function () {
        return this.value;
    }).get();
    ...
    var time_out = $('input[name="time_out[]"').map(function(){
        return this.value;
    }).get();
    ...
    var working_hours = $('input[name="working_hour[]"]').map(function(){
      return this.value;
    }).get();
    ...
    profile_expiry = $('#profile_expiry').val();
    ext_hour = $('#ext_hour').val();
    ext_min = $('#ext_min').val();
    ext_frequency = $('#ext_frequency').val();
    july_start = $('#july_start').val();
    sat_working_hours = $('#sat_working_hours').val();
    saturday_on = $('#saturday_on').val();
   
   
    

    // Ajax for constructing profile detail
    ...
    $.ajax({
      ...
      data:{
          ...
          sat_working_hours:sat_working_hours,
          saturday_on:saturday_on,
          },
      url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/insert_profile",
      success:function(e){
        last_inserted_id = e;

        // Ajax for constructing days profile
        $.ajax({
          type:"POST",
          cahce:false,
          data:{
          ...
          url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/insert_week_days",
          success:function(e){
            //console.log(e);
          }
        });

        // Ajax for constructing Allocation to Profile

        // var staff_id =  $('input[name=staffCheck]:checked').map(function(){
        //     return this.value;
        // }).get();

        var staff_id = [];
        var id = "";
        var oTable = $("#StaffListHere").dataTable();
        $(".selectAll:checked", oTable.fnGetNodes()).each(function() {
            if (id != "") {
                id = id + "," + $(this).val();
        ...
        if(staff_id.length  > 0){
          ...
          url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/insert_staff_allocation",
          success:function(e){

          console.log(e);

          // Get Profile from Profile Setup

           $.ajax({
           type:"POST",
           cache:false,
           url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/get_profile",
           success:function(f){
            $('#left-replace').html(f);
  ...
  // ===================Inserted Calculated Working Hours ==============//
  //====================================================================//

  $(document).on('keyup change','#time_out,#time_in',function(){

    var allocation =   $(this).attr('data-id');
    var time1 = $(".time_in_"+allocation).val().split(':'), time2 = $(".time_out_"+allocation).val().split(':');
    ...
    if(hours == 'NaN'){
      $(".working_hour_"+allocation).val('');
    }else{
      $(".working_hour_"+allocation).val(hours);
    }
  ...
  $(document).on('click','#profile_click',function(){
      ...
      $.ajax({
        ...
        url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/edit_profile",
        success:function(e){
          // console.log(e);
          $('.view_add').html(e);
        }
  ...
  $(document).on('click','#edit_profile',function(e){
    ...
    var profile_update_name = $('#profile_update_name').val();
    var profile_update_expiry = $('#profile_update_expiry').val();
    var ext_update_hour = $('#ext_update_hour').val();
    var ext_update_min = $('#ext_update_min').val();
    var ext_update_frequency = $('#ext_update_frequency').val();
    var july_update_start = $('#july_update_start').val();
    var sat_working_hours_update = $('#sat_working_hours_update').val();
    var saturday_on_update = $('#saturday_on_update').val();
    var saturday_off_update = $('#saturday_off_update').val();

    

    var time_in_update = $('input[name="time_in_update[]"]').map(function(){
      return this.value;
    }).get();



    var time_out_update = $('input[name="time_out_update[]"]').map(function(){
      return this.value;
    }).get();

    var working_hour_update = $('input[name="working_hour_update[]"]').map(function(){
      return this.value;
    }).get();

    var week_day_update = $('input[name="days_update[]"').map(function(){
      return this.value;
    }).get();
    ...
    // var staff_id =  $('input[name=staffCheckupdated]:checked').map(function(){
    //         return this.value;
    // }).get();

    // $('#calltable tbody tr:has(input:checkbox:checked)').each(function(){
    ...
    $(".checkItem1:checked", oTable.fnGetNodes()).each(function() {
        if (id != "") {
            id = id + "," + $(this).val();
            staff_id.push($(this).val());
        } else {
    ...
    $.ajax({
      ...
      data:{
        ...
        profile_update_expiry:profile_update_expiry,
        ext_update_hour:ext_update_hour,
        ext_update_min:ext_update_min,
        ext_update_frequency:ext_update_frequency,
        july_update_start:july_update_start,
        sat_working_hours_update:sat_working_hours_update,
        saturday_on_update:saturday_on_update,
      
      },
      url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/update_profile_setup",
      success:function(e){
       ...
       $.ajax({
        ...
        data:{
          profile_id:profile_id,
          time_in_update:time_in_update,
          time_out_update:time_out_update,
          working_hour_update:working_hour_update,
          week_day_update:week_day_update
        },
        url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/edit_week_days",
        success:function(f){
       ...
       $.ajax({
        ...
        url:"<?php echo base_url() ?>index.php/tif_a/tt_profile_ajax/edit_staff_allocation",
        success:function(s){
  ...
  // =========Edi Calculated Working Hours==========//
  //============================================//

   $(document).on('keyup change','#time_out_update , #time_in_update',function(){

    var allocation =   $(this).attr('data-id_update');
    var time1 = $(".time_in_update_"+allocation).val().split(':'), time2 = $(".time_out_update_"+allocation).val().split(':');
    var hours1 = parseInt(time1[0], 10),
    hours2 = parseInt(time2[0], 10),
    mins1 = parseInt(time1[1], 10),
    mins2 = parseInt(time2[1], 10);
    ...
    if(hours == 'NaN'){
      $(".working_hour_update_"+allocation).val('');
    }else{
      $(".working_hour_update_"+allocation).val(hours);
    }
...
//====================Check Box Profile Expiry===================//
//===============================================================//

$(document).on('change','.ProfileExpiry, .ExtensionInProfile, .CustomTiming',function(){
  ...
  if($('.ProfileExpiry').is(":checked")){   
    ...
    $('#profile_expiry').val('');
    $('#profile_update_expiry').val('');
    $(".ProfileExpiryYes").hide();
  }

  // For Extension In Profile
  if($('.ExtensionInProfile').is(":checked")){
    ...
    $('#ext_update_hour').val('');
    $('#ext_update_min').val('');
    $('#ext_update_frequency').val('');
    $('#july_update_start').val('');   
    $(".ExtensionYes").show();
  }
  else{
    $(".ExtensionYes").hide();
  ...
  if($('.CustomTiming').is(":checked")){
    $('.CutomTimingWrapper').show();
  }
  else{
    $('.CutomTimingWrapper').hide();
  }
  ...
  //============== Only Numeric Keys is Pressed =============//
  //=========================================================//
$(document).on('keypress','#sat_working_hours,#saturday_on,#sat_working_hours_update,#saturday_on_update',function(evt) {
...
$(document).on('click','#greenBTN3',function(){
  ...
  $('#profile_update_name').val('');
  $('#profile_update_expiry').val('');
  $('#sat_working_hours_update').val('');
  $('#saturday_on_update').val('');
  $('#ext_update_hour').val('');
  $('#ext_update_min').val('');
  $('#ext_update_frequency').val('');
  $('#july_update_start').val('');
...
// // New Updation In Morning And AfterNoon Time

// $(document).on('keyup change','#morning_time,#afternoon_time',function(){
//   var morning_time = $('#morning_time').val();
//   var afternoon_time = $('#afternoon_time').val();
//   $('.morning').val(morning_time);
//   $('.afternoon').val(afternoon_time);

//   //var allocation =   $(this).attr('data-id_update');
//     var time1 = $('#morning_time').val().split(':'), time2 = $('#afternoon_time').val().split(':');
//     var hours1 = parseInt(time1[0], 10),
//     hours2 = parseInt(time2[0], 10),
//     mins1 = parseInt(time1[1], 10),
//     mins2 = parseInt(time2[1], 10);
...
//     if(hours == 'NaN'){
//       $(".working_hour_allocation").val('');
//     }else{
//       $(".working_hour_allocation").val(hours);
//     }
...
// // New Updation In Wed AfterNoon

// $(document).on('keyup change','#wed_afternoon_time',function(){
//     var morning_time = $('#morning_time').val();
//     var wed_afternoon_time = $('#wed_afternoon_time').val();

//     $('.time_out_3').val(wed_afternoon_time);
//     var time1 = $('#morning_time').val().split(':'), time2 = $('#wed_afternoon_time').val().split(':');
//     var hours1 = parseInt(time1[0], 10),
//     hours2 = parseInt(time2[0], 10),
//     mins1 = parseInt(time1[1], 10),
//     mins2 = parseInt(time2[1], 10);
...
// // New Updation In Friday AfterNoon

// $(document).on('keyup change','#fri_afternoon_time',function(){
//     var morning_time = $('#morning_time').val();
//     var fri_afternoon_time = $('#fri_afternoon_time').val();

//     $('.time_out_5').val(fri_afternoon_time);
//     var time1 = $('#morning_time').val().split(':'), time2 = $('#fri_afternoon_time').val().split(':');
//     var hours1 = parseInt(time1[0], 10),
//     hours2 = parseInt(time2[0], 10),
//     mins1 = parseInt(time1[1], 10),
//     mins2 = parseInt(time2[1], 10);
