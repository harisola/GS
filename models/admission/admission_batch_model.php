...
class Admission_batch_model extends CI_Model{
	...
	public function  get_result_discussion($form_id,$form_status_id){
		 ...
		 $query = "Select af.form_no,af.official_name,af.form_status_id,af.form_batch_id,af.batch_slot_id,
		af.form_assessment_result,af.form_assessment_result_by,af.form_assessment_date,
		af.form_assessment_decision_by,
	  ...
	  IF(af.form_assessment_decision = '',0,af.form_assessment_decision) AS form_assessment_decision,
		af.conditional_unconditional,
	...
	public function  get_result_admission($form_id){

		$query = "Select af.form_no AS form_no,af.official_name,af.form_status_id,af.form_batch_id,af.batch_slot_id,
		af.form_assessment_result,af.form_assessment_result_by,af.form_assessment_date,
		...
		inner join atif_gs_admission._form_status fs on fs.id = af.form_status_id
		left join (select * from atif_gs_admission.admission_form_waitlist) as afw on afw.admission_form_id = af.id
	...
	public function get_aso_and($batch_id){
		 $query = "select
				...
				pad.form_discussion_decision_by as pvs_form_decision_by,
				pad.form_assessment_result as pvs_form_assessment_result,
				pad.form_discussion_decision_by as pvs_form_discussion_decision_by,
				pad.form_assessment_result_by as pvs_form_assessment_result_by,
				pad.form_assessment_decision as pvs_form_assessment_decision,
				pad.form_discussion_date as pvs_form_discussion_date,
				pad.form_discussion_time as pvs_form_discussion_time,
				pad.form_discussion_result_by as pvs_form_discussion_result_by,
				...
				/***** RE-Discussion *****/
				IFNULL(CONCAT(DATE_FORMAT(af.form_discussion_date, '%a %d %b'), '@ ', TIME_FORMAT(af.form_discussion_time, '%h:%i')), '') as form_discussion_date,
				IFNULL(DATE_FORMAT(RDis_Presence.date, '%a %d %b'),'') AS rdis_done_on,
				IF(IFNULL(RDIS_Reminder.admission_form_id,0)=0,0,1) as flag_rdis_reminder, IF(IFNULL(RDIS_Presence.date,0)=0,0,1) as flag_rdis_presence, 
				IF(IFNULL(RDIS_Followup.admission_form_id,0)=0,0,1) as flag_dis_followup,
				af.form_discussion_result,
				IF(af.form_discussion_result='',0,1) as flag_rdis_result,
				IF(af.form_discussion_decision='' or af.form_discussion_decision='RST' ,0,1) as flag_rdis_decision,
				IF(af.form_discussion_decision='' or af.form_discussion_decision='RST' ,0,1) as flag_rdis_allocation,
				IF(IFNULL(OFR_Communication.admission_form_id,0)=0,0,1) as flag_comm_dis_result,
				/***** RE-Discussion - END *****/
				...
				left join (select
					admission_form_id, from_unixtime(modified) as date
					from atif_gs_admission.log_form_status
					where new_form_status = 8
					and new_form_stage = 4
					/*and type = 'G'*/
					group by admission_form_id) as RST_Presence
						on RST_Presence.admission_form_id = af.form_id
				...
				left join (select
					...
					and new_form_stage = 3
					/*and type = 'G'*/
					group by admission_form_id) as RST_Reminder
						on RST_Reminder.admission_form_id = af.form_id
				...
				left join (select
					...
					and (new_form_stage = 5 or new_form_stage = 7 or new_form_stage = 10 or new_form_stage = 11)
					/*and type = 'G'*/
					group by admission_form_id) as RST_Followup
						on RST_Followup.admission_form_id = af.form_id
				...
				left join (select
					...
					and new_form_stage = 2
					/*and type = 'G'*/
					group by admission_form_id) as RDIS_Communication
						on DIS_Communication.admission_form_id = af.form_id
				/***** Reassistment - END *****/
				...
				left join (select
					admission_form_id, from_unixtime(modified) as date
					from atif_gs_admission.log_form_status
					where new_form_status = 9
					and new_form_stage = 4
					/*and type = 'G'*/
					group by admission_form_id) as RDis_Presence
						on RDis_Presence.admission_form_id = af.form_id
				...
				left join (select
					...
					and new_form_stage = 3
					/*and type = 'G'*/
					group by admission_form_id) as RDIS_Reminder
						on DIS_Reminder.admission_form_id = af.form_id
				...
				left join (select
					...
					and (new_form_stage = 5 or new_form_stage = 7 or new_form_stage = 10 or new_form_stage = 11)
					/*and type = 'G'*/
					group by admission_form_id) as RDIS_Followup
						on RDIS_Followup.admission_form_id = af.form_id
				...
				left join (select
					...
					and new_form_stage = 2
					/*and type = 'G'*/
					group by admission_form_id) as ROFR_Communication
						on ROFR_Communication.admission_form_id = af.form_id
				/***** RE-Discussion - END *****/
				...
				left join atif_gs_admission.previous_admission_details pad
				
				ON pad.form_number=af.form_no
	...
	public function get_result_admission_new_by_form_id($form_id){
				$query = "SELECT 
				vaf.form_id,af.form_no,af.form_batch_id,
				af.batch_slot_id,vaf.form_batch_id,vaf.batch_slot_id,
				vaf.form_assessment_date,vaf.form_assessment_date,
				vaf.form_discussion_date
				,vaf.form_assessment_result,
				vaf.form_assessment_decision,
				vaf.form_discussion_time,
				vaf.form_discussion_time,
				vaf.form_discussion_result,
				af.form_discussion_decision_by,
				vaf.form_discussion_decision,
				vaf.ast_name_code,vaf.dis_name_code
				FROM atif_gs_admission.view_admission_form vaf 
				INNER JOIN atif_gs_admission.admission_form af
				on af.id=vaf.form_id
				where vaf.form_assessment_date != '0000-00-00'
				and vaf.form_assessment_date != '2001-01-01'
				and vaf.form_status_id != 1
				and vaf.form_id  = $form_id /*order by af.batch_time_submission*/";
				$row = $this->db->query($query);
				return $row->result();
	}
	...
	public function get_aso_and_previous($batch_id){
			    $query = "select
				af.form_id,af.form_no, af.official_name as applicant_name, af.father_name,pad.form_number,
				pad.previous_batch_id,pad.previous_slot_id,pad.form_assessment_date,pad.form_discussion_decision_by,
				pad.form_assessment_date as pvs_form_assestment_date,
				pad.ast_name_code as pvs_ast_name_code,
				pad.dis_name_code as pvs_dis_name_code,
				pad.form_discussion_decision_by as pvs_form_decision_by,
				pad.form_assessment_result as pvs_form_assessment_result,
				pad.form_discussion_decision_by as pvs_form_discussion_decision_by,
				pad.form_assessment_result_by as pvs_form_assessment_result_by,
				...
				pad.form_discussion_time as pvs_form_discussion_time,
				pad.form_discussion_result_by as pvs_form_discussion_result_by,
				pad.form_discussion_result as pvs_form_discussion_result,
				pad.form_discussion_decision as pvs_form_discussion_decision,
				...
				/***** Assessment *****/
				CONCAT(DATE_FORMAT(pad.form_assessment_date, '%a %d %b'), '@ ', TIME_FORMAT(af.batch_time_submission, '%h:%i')) as form_assessment_date,
				IFNULL(DATE_FORMAT(AST_Presence.date, '%a %d %b'),'') AS ast_done_on, af.ast_name_code, 
				IF(IFNULL(AST_Reminder.admission_form_id,0)=0,0,1) as flag_ast_reminder, IF(IFNULL(AST_Presence.date,0)=0,0,1) as flag_ast_presence, 
				IF(IFNULL(AST_Followup.admission_form_id,0)=0,0,1) as flag_ast_followup,
				af.form_assessment_result, af.form_assessment_decision,
				...
				/***** Assessment *****/
				IFNULL(CONCAT(DATE_FORMAT(pad.form_discussion_date, '%a %d %b'), '@ ', TIME_FORMAT(af.form_discussion_time, '%h:%i')), '') as form_discussion_date,
				IFNULL(DATE_FORMAT(Dis_Presence.date, '%a %d %b'),'') AS dis_done_on, af.dis_name_code,
				IF(IFNULL(DIS_Reminder.admission_form_id,0)=0,0,1) as flag_dis_reminder, IF(IFNULL(DIS_Presence.date,0)=0,0,1) as flag_dis_presence, 
				IF(IFNULL(DIS_Followup.admission_form_id,0)=0,0,1) as flag_dis_followup,
				af.form_discussion_result, af.form_discussion_decision,
				...
				and af.form_assessment_date != '2001-01-01'
				and af.form_status_id != 1
				and pad.previous_batch_id  = $batch_id /*order by af.batch_time_submission*/";
				$row = $this->db->query($query);
				return $row->result();
	}


	public function checkReAssistmentProcess($batch_id){
		$query = "select * from atif_gs_admission.previous_admission_details pad where pad.previous_batch_id=$batch_id";
		$row = $this->db->query($query);
				return $row->result();
	}
	...
	public function get_and_detail($form_no){
		$query = "select
		af.form_id, LPAD(af.form_no, 5, '0') AS form_no, af.official_name as applicant_name, af.father_name,
	...
	public function getbatchadmissiondetail($batch_id){
		...
		// Batch Sheet
	public function get_rubiks_discussion($batch_id,$Form_id){
		...
		WHERE lgs.new_form_status >= 6
		GROUP BY lgs.admission_form_id) AS outcome ON outcome.admission_form_id = af.form_id /***** Outcome date - End *****/
		WHERE af.form_assessment_date != '0000-00-00' AND af.form_assessment_date != '2001-01-01' AND af.batch_category = '$batch_id'  and af.form_id=".$Form_id."
		ORDER BY af.batch_time_submission) AS DATA
		GROUP BY form_no
		ORDER BY batch_time_submission, form_no";
...
public function getbatchadmissiondetailPN_N($form_id){
		...
		WHERE lgs.new_form_status >= 6
		GROUP BY lgs.admission_form_id) AS outcome ON outcome.admission_form_id = af.form_id /***** Outcome date - End *****/
		WHERE af.form_assessment_date != '0000-00-00' AND af.form_assessment_date != '2001-01-01' AND af.form_id = ".$form_id."
		ORDER BY af.batch_time_submission) AS DATA
		GROUP BY form_no
		ORDER BY batch_time_submission, form_no";
	...
	public function get_dis_presence($form_id){
		...
		$query = "select
		af.id as form_id, LPAD(af.id, 5, '0') AS form_no, af.official_name as applicant_name,

		IF(IFNULL(AST_Presence.date,0)=0,0,1) as flag_dis_presence, 
		IF(IFNULL(RST_Presence.date,0)=0,0,1) as flag_rdis_presence, 
		...
		left join (select
			...
			and new_form_stage = 4
			group by admission_form_id) as RST_Presence
				on RST_Presence.admission_form_id = af.id
	...
	public function get_setup_batch($grade_id){
		$query = "select CONCAT(dobg.batch_code,'-','01') as batch_name,grade_id as num from atif_gs_admission._dob_grade as dobg where dobg.grade_id =$grade_id";
		$row = $this->db->query($query);
		return $row->result();
	}

	public function insertPreviousSlotData($data){
		$this->db_studentAdmission->set($data);
	    $this->db_studentAdmission->insert('previous_admission_details');
	}

	public function UpdateAdmissionForm($data,$form_id){
		$this->db_studentAdmission->where('id',$form_id);
		$this->db_studentAdmission->set($data);
		$this->db_studentAdmission->update('admission_form');
	}

	public function UpdateSlot($form_id,$p_batch_id,$p_slot_id,$n_batch_id,$n_slot_id){

		$query = "UPDATE _form_batch_slots SET revised_batch_slot_id=$n_slot_id where id=$p_slot_id";
		$this->db->query($query);

		$query = "UPDATE _form_batch_slots SET admission_form_id=$form_id where id=$n_slot_id";
		$this->db->query($query);


	}

	public function updateDiscussionStatus($form_id,$status){
		$query = "UPDATE admission_form SET form_discussion_decision='$status' where id=$form_id";
		$this->db->query($query);
	}
