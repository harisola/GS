...
$(document).on("click", ".mutliSelectG2", function(){
	var batch_id = $(this).val();
	//alert(batch_id);
	if($(this).is(':checked')){
		var appendHidden2 = '<input type="hidden" class="batch_cat_list" name="batch_cat_name[]" value="'+batch_id+'" id="batch_cat_id_'+batch_id+'" />';
		$('#fliter_issuance').append(appendHidden2);
	}else{
		$("#batch_cat_id_"+batch_id+"").remove();
	}
});
...
var ID = $(this).closest('.mutliSelect').find('input[type="checkbox"]').val(),
	...
	if($(this).is(':checked')){
		...
		$(".hidaG").hide();
		//var appendHidden2 = '<input type="hidden" class="someElement" name="batch_cat_name[]" value="'+ID+'" id="batch_cat_id_'+ID+'" />';
		//$('#fliter_issuance').append(appendHidden2);
	}else {
...
var formdis_dec = $("#discussion_decission");
...
formdis_dec.append( '<option value="'+el+'">'+el+'</option>' )
...
function getvalues(){
	...
	//var BiGSelectBox = $(".BiGSelectBox").val();
		...
		var batch_category = "";
		$('.batch_cat_list').each(function(){
			if( $(this).val() != '' ){
				if( blean ){
					batch_category += $(this).val();	
					blean= false;
				}else{
					batch_category +=","+$(this).val();
				}
