...
<!-- Student Penalty Modal -->
<div class="modal fade" id="student_penalty_modal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">Student Penalty</h4></div>
      <div id="modal_body_studentpenalty" class="modal-body text-center ativa-scroll"></div>                                       
...
<!-- Student Pending Absent Modal -->
<div class="modal fade" id="student_pending_absent_modal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">Absent Pending Case</h4></div>
      <div id="modal_body_pendingabsent" class="modal-body text-center ativa-scroll"></div>                                       
...
<div class="modal fade" id="student_activecase_modal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button id="activecase_button" type="submit" class="btn btn-orb-org" data-dismiss="modal">Done</button>
      </div>
    </div>
    <!-- /.modal-content --> 
  </div>
...
$(document).on("click", ".show_student_activecase", function (e) {
    ...
    $.post('<?php echo base_url() . "index.php/student_attendance/activecase_current_modal/";?>?gsid='+gsid, function(data) {
...
$(document).on("click", ".show_student_pending_absent", function (e) {
    e.preventDefault();
    var _self = $(this);
    var gsid = _self.data('id');    
    $.post('<?php echo base_url() . "index.php/student_attendance/activecase_pending_absent_modal/";?>?gsid='+gsid, function(data) {
      $('#modal_body_pendingabsent').html(data);
    });
    $("#student_pending_absent_modal").modal('show');
});


$(document).on("click", ".show_student_penalty_history", function (e) {
    e.preventDefault();
    var _self = $(this);
    var gsid = _self.data('id');    
    $.post('<?php echo base_url() . "index.php/student_attendance/activecase_penalty_modal/";?>?gsid='+gsid, function(data) {
      $('#modal_body_studentpenalty').html(data);
    });
    $("#student_penalty_modal").modal('show');
});
...
function setUpdatepresent() {
    document.frmUserpresent.action = "<?php echo site_url()?>/student_attendance/atd_today/add";
    document.frmUserpresent.submit();
}

function setUpdatepresentfence() {
    document.frmUserpresentfence.action = "<?php echo site_url()?>/student_attendance/atd_today/add";
    document.frmUserpresentfence.submit();
}

function setUpdateabsent() {
    document.frmUserabsent.action = "<?php echo site_url()?>/student_attendance/atd_today/add2";
    document.frmUserabsent.submit();
}


$("#activecase_button").click( function()
   {
      if($("#activecase_id").val() > 0){
        var gs_id = $("#gs_id").val();
        var comments = $("#comments").val();
        var activecase_id = $("#activecase_id").val();
        
        $.post("<?php echo site_url() . '/student_attendance/atd_history/add2'; ?>", //Required URL of the page on server
        { // Data Sending With Request To Server
          gs_id:gs_id,
          activecase_id:activecase_id,
          comments:comments
        });
      }

      if($("#activecase_category").val() > 0){
        var gs_id = $("#gs_id").val();
        var activecase_category = $("#activecase_category").val();        
        
        $.post("<?php echo site_url() . '/student_attendance/atd_history/add'; ?>", //Required URL of the page on server
        { // Data Sending With Request To Server
          gs_id:gs_id,
          activecase_category:activecase_category          
        });
      }      
   }
);
