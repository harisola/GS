  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Grade-Level Programme | Option Defination</h4></div>
      <div id="modal-body" class="modal-body text-center"><h5>Data Already Inserted</h5></div>                                       
...
$('#academic-change').on('change',function(){
var value =$('#academic-change option:selected').text();
...
function fetch_SC()
{
    $.ajax({
        type:"POST",
        url:"<?php echo base_url(); ?>index.php/program_group/subject_grade_ajax/get_SC",
        cache:false,
        data:"json",
        success:function(e)
        {
            var JsonData = JSON.parse(e);
            var a = "<option value = '' " + " selected disabled >" + "Grade Name" + "</option>";
            for(var i=0 ; i<JsonData.length ;i++)
            {
                a += "<option value = '" + JsonData[i].id + "'" +">" + JsonData[i].name + "</option>";
            }
            $('#grade').html(a);
        }
    });
...
function fetch_NC()
{
    $.ajax({
        type:"POST",
        url:"<?php echo base_url(); ?>index.php/program_group/subject_grade_ajax/get_NC",
        cache:false,
        data:"json",
        success:function(e)
        {
...
if ($('#academic-subject').length) {
            $("#academic-subject").validate({
                ...
                messages: {
                    academic: {
                        required: 'Please select the Academic Name'
                    },
                    grade: {
                        required: 'Please select the Grade Name'
                    }
                ...
                submitHandler: function (form) {
                    $(form).ajaxSubmit({
                        type: "POST",
                        url:"<?php echo base_url() ?>index.php/program_group/subject_grade_ajax/get_teacher",
                        cache:false,
