...
class Notify_log_ajax extends CI_Controller {
	...
	public function get_converssion(){
		...
		else
		{
			...
			array_push( $tableRows, $notify_id );
  ...
  return $html;
	...
	public function Admission_Form( $form_id  )
	{
		...
		$data["student_name"] = $fInfo["call_name"];
		$data["formInfo"] = $fInfo;
		$data["fComt"] = $this->ff->getFormComments($form_id);
		
		switch( $fInfo["form_status_id"] )
		{
			case 1: $currentStage = "Submission"; break;
			case 2: $currentStage = "Assessment"; break;
			case 3: $currentStage = "Discussion"; break;
			case 4: $currentStage = "Offer"; break;
			case 5: $currentStage = "Offer"; break;
		}
		...
		$Current_time = time();
		$Custom_Query = "UPDATE `atif_notification`.`notify_meta_data` SET `is_read`='1',`modified`=".$Current_time.",`modified_by`=".$User_id." WHERE  `log_id`=".$form_id." AND `notify_to`=".$User_id."";
		
		$this->CL->Custom_Query( $Custom_Query );
