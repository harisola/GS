...
class Fee_bill_upload_mis extends MY_Controller {
		...
		if(isset($_POST["submit"]))
		{
			...
			{	
				...
				for ($i=$titleRow+1; $i<=$totalRecords; $i++){
					...
					if($uploadedGBID != 81 && $uploadedGBID != 82 && $uploadedGBID != 72){
						...
						if($FeeBillReceivingID == 0){
							...
							if($uploadedGBID == 81 || $uploadedGBID == 82){
								...
								if($this->adm->ShouldRegisterStudent(substr($thisGBID, 5, 4))){
									if(substr($thisGBID, 2, 2) != "72"){
										$this->RegisterThisStudent(trim($data['values'][$i]['G']), $upload_date, $upload_bank_branch);									
									}
								}else{
									$FormNo = substr(trim($data['values'][$i]['G']), 5, 4);
									$this->Insert_MIS_Log(trim($data['values'][$i]['G']), $upload_date, $upload_bank_branch, $FormNo);
								}
							}
	...
	public function RegisterThisStudent($FeeBillID, $ReceivedDate, $ReceivedBranch){
			...
			* *********************************************************************************/
			...
			if(!$GRNoExists){
				if($alreadyRegisteredStd == 0){
					...
					$GSID = $this->student_registered_model->getOldStudentGSID($OfficialName, $DOB);
				}
			}else{
			...
			* *********************************************************************************/			
			...
			$data = array(
				...
				'house_id' => 1,
				'status_id' => 3,
				'rfid_dec_no' => $GSID.'xyz',
				'rfid_hex_no' => $GSID.'xyz',
			...
			* *********************************************************************************/
			...
			$this->Insert_MIS_Log($FeeBillID, $ReceivedDate, $ReceivedBranch, $FormNo);
	...
	public function Insert_MIS_Log($FeeBillID, $ReceivedDate, $ReceivedBranch, $FormNo){
		$this->load->model('front_office/Family_registered_model');
