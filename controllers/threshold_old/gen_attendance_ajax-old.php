...
class Gen_attendance_ajax extends CI_Controller {
	...
	public function get_TapIn_Information()
	{
		...
		if(!empty($TapinData)){
			...
			/*
			if($TapinData[0]['rfid_dec'] == $RFID_Dec){
				$name = $TapinData[0]['name'];
				$nic  = $TapinData[0]['nic'];
				$mobile_phone  = $TapinData[0]['mobile_phone'];
				$gender = 'F';
			}else{
				$name = $TapinData[1]['name'];
				$nic  = $TapinData[1]['nic'];
				$mobile_phone  = $TapinData[1]['mobile_phone'];
				$gender = 'F';
			}*/


			/*
			$SiblingsData = $this->student_basic_profile_model->get_siblings_data_GFID($TapinData[0]['gf_id']);
