...
class Notify_header{
	...
	public function group_menus_count(){
		...
		if( $unread_list = $this->ci->NTF->get_unread_notification($userid) ){}else{ $unread_list = FALSE; }
		if( !empty( $unread_list )){
			if( count( $unread_list ) > 0 ){ $countN = count( $unread_list ); }else{ $countN =0; }
		}else{ $countN = 0; }
	...
	public function notify_list2($countN,$isTeacher=NULL){
		...
		$html .= '<button type="button" class="btn btn-default" data-toggle="dropdown">';
		$html .= '<i class="entypo-megaphone"></i>';
		$html .= '<span class="newNofitication" id="newNofitication" style="display: block;"> '. $countN.' </span>';	
		$html .= '</button>';
		$html .= '<div id="notification-dropdown" class="dropdown-menu">';
