...
class Master_calendar_ajax extends CI_Controller{
	...
	*/
	...
	function HParamter(){
		...
		$html .= '<h4 class="modal-title">Holiday Parameter</h4>';
		$html .= '</div>';
		...
		$html .= '<input type="submit" class="greenBTN widthSmall" value="Set Holiday Parameter" id="HPButton" />';
		
		if( $HChecking==1){
			$html .= ' <button type="button" class="btn btn-default" data-type="HP" data-dated="'.$IDate.'" data-id="'.$dataid.'" id="HPRevome">Remove! Parameter</button>';
		}
	...
	*/
	function HParamterH(){
		...
		$dataid = $this->input->post("dataid");
		$MethodT = $this->input->post("MethodT");
		$HijriDateBox = $this->input->post("Hijri_Date"); // WenBox_2017_12-20
		$IDate ='';
		if( $MethodT == 'Hlpara'){
			//insertation
			$meth = 2;
		...
		$html .= '<div class="col-md-9">';
			...
			$e = explode("_", $HijriDateBox);
			
			if( !empty($e))
			{
				$Schedule_hidden_date = date("Y-m-d",strtotime($e[1]));
			}
			else
			{
				$Schedule_hidden_date = date("Y-m-d");
			}
			
			
			$html .= '<input type="hidden" value="'.$Schedule_hidden_date.'" name="Schedule_hidden_date" id="Schedule_hidden_date" />';
		...
		if( $HChecking==1){
			$html .= ' <button type="button" class="btn btn-default" data-type="HP_H" data-dated="'.$IDate.'" data-id="'.$HijriDateBox.'" id="HPRevome">Remove! Parameter</button>';
		}
	...
	function SfParamterH(){
		...
		$dataid = $this->input->post("dataid");
		$MethodT = $this->input->post("MethodT");
		$Parent_Container_date = $this->input->post("Hijri_Date");
		$ee = explode("_", $Parent_Container_date);
		if( !empty($ee))
		{
			$Schedule_hidden_date = date("Y-m-d", strtotime($ee[1]));
		}	
		else
		{
			$Schedule_hidden_date = date("Y-m-d");
	...
	<div class="col-md-12" style="padding:10px 0;">
		...
		<div class="col-md-9">
			...
			$html .= '<input type="hidden" value="'.$dataid.'" name="date_id_SP" id="date_id_SP" />';
			$html .= '<input type="hidden" value="'.$meth.'" name="OpType" id="OpType" />';
			$html .= '<input type="hidden" value="'.$Parent_Container_date.'" name="HijriDateBox" id="HijriDateBox" />';
			$html .= '<input type="hidden" value="'.$Schedule_hidden_date.'" name="Schedule_hidden_date" id="Schedule_hidden_date" />';
		...
		if( $HChecking==1){
			$html .= ' <button type="button" class="btn btn-default" data-type="SP_H" data-dated="'.$IDate.'" data-id="'.$Parent_Container_date.'" id="HPRevome">Remove! Parameter</button>';
		}
	...
	function SchFParameter(){
		...
		$Std_Dated = '';
		$HijriDate3 = $this->input->post("Hijri_Date"); 
...
<div class="row">
		...
		<div class="col-md-6 no-padding" style="margin-top: 13px !important;border-right: 1px solid #ccc;height: 361px;">
			...
			<div class="col-md-12" style="padding:10px 0;">
				...
				if($addfollow==1){
				$html .='<input type="checkbox" name="addfollow" value="1" id="addfollow" checked disabled> <label for="addfollow"> Add followup to the respective date</label>';	
				}else{
		...
		$html .= '</div></div>';
			...
			$html .= $this->Get_Stff_Profile($Type='NE');
...
$html .='</div>';
$html .='<div class="modal-footer">';
  $html .='<input type="submit" class="greenBTN widthSmall"  value="Set Followup" id="SetFollowup" />';
  if( $meth==2){
	  $html .= '<input type="hidden" name="HijriDateBox" value="'.$HijriDate3.'" id="HijriDateBox" />';	
$html .= ' <button type="button" class="btn btn-default" data-type="SCHP" data-dated="'.$Std_Dated.'" data-id="'.$dataid.'" id="HPRevome">Remove! Parameter</button>';	
}
...
$html .='</div>';
	...
	$html .= '<input type="hidden" name="Schedule_hidden_date" value="'.$dated2.'" id="Schedule_hidden_date"  />';
...
function AddNewEventModalHtml(){
	...
	$Hijri_Date_box = $this->input->post("Hijri_Date");
...
<div class="row">
	...
	$html .= '</div>';
		...
		$html .= '</div>';
		$html .= '<div class="modal-footer">';
		$html .= '<input type="submit" class="greenBTN widthSmall" value="Add Event" id="AddEventButton">';
		if( $meth==2){
		$html .= '<input type="hidden" name="HijriDateBox" value="'.$Hijri_Date_box.'" id="HijriDateBox" />';	
		$html .= ' <button type="button" class="btn btn-default" data-type="Evnt" data-dated="'.$Hijri_Date_box.'" data-id="'.$dataid.'" id="HPRevome">Remove! Parameter</button>';	
		}
		$html .= '</div>';
...
<div class="row">
	...
	$html .= '</div>';
		...
		$html .= '</div>';
		$html .= '<div class="modal-footer">';
		$html .= '<input type="submit" class="greenBTN widthSmall"  value="Add Hijri Event" id="AddEventButtonHijri">';
		
		if( $meth==2){
		$html .= '<input type="hidden" name="HijriDateBox" value="'.$Hijri_Date_box.'" id="HijriDateBox" />';	
		$html .= ' <button type="button" class="btn btn-default" data-type="Evnt_H" data-dated="'.$Hijri_Date_box.'" data-id="'.$dataid.'" id="HPRevome">Remove! Parameter</button>';	
		}
	...
	public function Get_Stff_Profile($Type){
		
		$this->load->model("master_calendar/master_calender","MC");
		$Wings = $this->MC->Get_Staff_Profile();
		...
		foreach( $Wings as $Profile):
		
		/*if( $Profile['department_ID'] == $Profile['Profile_id'] ){
			$Checked='checked';
		}else{
			$Checked='';
		}*/
		...
		$html .= '<div class="panel panel-default">
							<div class="panel-heading" role="tab" id="headingSix">
								<h4 class="panel-title">
									<input name="Profile_name_'.$Profile['Profile_id'].'" value="'.$Profile['Profile_id'].'" id="Profile_name_id_'.$Profile['Profile_id'].'" type="checkbox" class="'.$ads_Checkbox.'" '.$Checked.' />
									<label for="Profile_name_id_'.$Profile['Profile_id'].'" class="customCheck"></label>
									<a class="titleColl collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseSix" aria-expanded="false" aria-controls="collapseSix">
										...
										<div class="col-md-10 no-padding">'.$Profile['Profile_name'].'</div>
									</a>
								</h4>
							</div><!-- panel-heading -->
							<!--
							<div id="collapseSix" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingSix" style="height: 0px;">
								<div class="panel-body">
									<ul class="noBullets">
										<li><input name="DI" value="DI" style="display:;" id="DI" type="checkbox"> <label for="DI" class="">DI</label></li>
									</ul>
								</div>
							</div> collapseThree -->
	...
	*/
	function db_hp(){
		...
		/*if( $this->Type == 'StaffP' || $this->Type == 'Schedule' ){
			if( $this->Type == 'Schedule' ){
				$From_date = $this->STitle;
				$To_date = $this->STitle;
			}else{
				$From_date = $DateID;
				$To_date = $DateID;
...
//var_dump(datediffInWeeks('1/2/2013', '6/4/2013'));// 21
	...
	function Remove_Parameter()
	{
		...
		if( $Hijri_Wise == 'H' )
		{
			$Table_name = 'calendar_events_managment_hijri_date';
			$Calendar_id = $Calendar_id;
			$where = array('ID' => $Calendar_id );
			$this->MC->table_update($Table_name, $Data, $where);
			$Query = "select e.id as Effected_id from atif_gs_events.event_effected_department_hijri_date e where e.event_ID=".$Calendar_id."";
			$Effected_id_list = $this->GetKashif($Query);
			if(!empty($Effected_id_list))
			{
				$Table_name = 'event_effected_department_hijri_date';
				$Calendar_id = $Calendar_id;
				$where_effect = array('event_ID' => $Calendar_id );
				$this->MC->table_update($Table_name, $Data, $where_effect);
			}
		...
		{
			$Table_name = 'calendar_events_managment';
			$Calendar_id = $Calendar_id;
			$where = array('ID' => $Calendar_id );
			$this->MC->table_update($Table_name, $Data, $where);
			$Query = "select e.id as Effected_id from atif_gs_events.event_effected_department e where e.event_ID=".$Calendar_id."";
			$Effected_id_list = $this->GetKashif($Query);
			if(!empty($Effected_id_list))
			{
				$Table_name = 'event_effected_department';
				$Calendar_id = $Calendar_id;
				$where_effect = array('event_ID' => $Calendar_id );
				$this->MC->table_update($Table_name, $Data, $where_effect);
			}
