...
/*
* Bellow method are belongs to holiday parameter for specific date 
* set, edit, and remove methods are defined
* after weeklytime sheet will be reset accordingly 
*/
...
$(document).on("click", "#HPRevome", function(){
	
var Calendar_id   	   = $("#date_id").val();
var Standard_date 	   = $("#Standard_date").val();
var Parents_Container  = '';
var Type = $(this).data("type");

if( Type == 'SCHP' )
{
	Parents_Container  = $("#HijriDateBox").val();
	Standard_date= $("#Schedule_hidden_date").val();
...
else if( Type == 'HP_H' )
{
	...
	Parents_Container  = $("#HijriDateBox").val();
	Standard_date= $("#Schedule_hidden_date").val();
	Calendar_id = parseInt( $(this).data("dated") );
}

else if( Type == 'Evnt' )
{
	
	Parents_Container  = $(this).data("dated");
	Standard_date= $("#AdNwEntDate").val();
	Calendar_id = parseInt( $(this).data("id") );
}
...
else if( Type == 'Evnt_H' )
{
	...
	Parents_Container  = $(this).data("dated");
	Standard_date= $("#AdNwEntDateHijri").val();
	Calendar_id = parseInt( $(this).data("id") );
...
var parent_Div = "#"+Parents_Container.toString();
var $rows = $(parent_Div);
var EL = $rows.find(".eventListingHere");
var EL2 = $rows.find(".col-md-12");
var Div_Anchors	='';
switch( Type )
{
	case 'HP':
		var Div_Anchors = $(EL).find('a.HolidayParameterSet').data("id",parent_Div);  // listing	
		var Div_Anchors2 = $(EL2).find('a.HolidayParameter3').data("id",parent_Div); // header
		RemoveClass = 'HolidayParameter3';
		AddClass = 'HolidayParameter';
		AddText = 'H';
		label_label='Holiday';
	...
	break;
	case 'HP_H': 
		var Div_Anchors = $(EL).find('a.HolidayParameterSet3').data("id",parent_Div);  // listing
		var Div_Anchors2 = $(EL2).find('a.HolidayParameter3').data("id",parent_Div); // header
		RemoveClass = 'HolidayParameter3';
		AddClass = 'HolidayParameter';
		AddText = 'H';
	...
	case 'SP' : 
		var Div_Anchors = $(EL).find('a.StaffParameterSet').data("id",parent_Div); 
		var Div_Anchors2 = $(EL2).find('a.StaffParameter3').data("id",parent_Div);
		RemoveClass = 'HolidayParameter3';
		AddClass = 'HolidayParameter';
		AddText = 'TP';
		label_label='Staff';
	...
	case 'Evnt' : 
		var Div_Anchors = $(EL).find('a.EditAddEvent').data("id",parent_Div); 
		//var Div_Anchors2 = $(EL2).find('a.AddEventLink').data("id",parent_Div);
		//RemoveClass = 'EditAddEvent';
		//AddClass = 'HolidayParameter';
		//AddText = 'TP';
		label_label='Event';
	break;
	...
	case 'Evnt_H' : 
		var Div_Anchors = $(EL).find('a.EdtAdEntH').data("id",parent_Div); 
		//var Div_Anchors2 = $(EL2).find('a.AddEventLink').data("id",parent_Div);
		//RemoveClass = 'EditAddEvent';
		//AddClass = 'HolidayParameter';
		//AddText = 'TP';
	...
	case 'SCHP' : 
		var Div_Anchors = $(EL).find('a.ScheduleFollowupSet').data("id",parent_Div); 
		var Div_Anchors2 = $(EL2).find('a.ScheduleFollowup3').data("id",parent_Div);
		RemoveClass = 'ScheduleFollowup3';
		AddClass = 'ScheduleFollowup';
		AddText = 'SF';
		label_label='Schedule';
	break;
 ...
 bootbox.dialog({
            message: "Are you sure you want to! Revomve Holiday Parameter?",
            title: "Please Confirm.",
            buttons: {
              confirm: {
                label: "Yes! Remove "+label_label,
                callback: function() { 
				
				Div_Anchors.each(function(i){
					$(this).delay(200*i).fadeOut(3000);
					$(this).animate({ "opacity" : "0",},{"complete" : function() { 
					
					Div_Anchors.remove(); 
					
					if( Type != 'Evnt'){
						Div_Anchors2.removeClass(RemoveClass);
						Div_Anchors2.addClass(AddClass);
						Div_Anchors2.text(AddText);
						Div_Anchors2.css("background-color", "");	
					}
					...
					Remove_Parameter(Type, Calendar_id, Standard_date)
					
					$('.modal.in').modal('hide');
					$('.SetHolidayParameter2.in').modal('hide');
					$("#HolidayParameters").html('');
					$("#StaffParameters").html('');
					//Update_Weekly_Time_Sheet(Standard_date, Standard_date);
...
function Remove_Parameter(Parameter_Type, Calendar_id, Dated)
{
...
//alert(Dated)
	...
	{	
		$.ajax({
			url: "<?=site_url()?>/master_calendar/master_calendar_ajax/Remove_Parameter",
			type: "POST",
			data:{Parameter_Type:Parameter_Type,Calendar_id:Calendar_id,Date:Date},
			dataType:'HTML',
			beforeSend: function(){},
			success: function(res){
				
				Update_Weekly_Time_Sheet(Dated, Dated);
			},
			complete: function(){}
		});
...
$(document).on("click", "#SetFollowup", function(){
	...
	if($("#addfollow").is(':checked')){
		...
		if( parseInt(count2) > 0 ){
			...
			$("#ScheduleFCurrentDate").val('');
			$("#ScheduleFCurrentDate").val(date_id2);
			appendToDiv( EL2, 'Schedule', anchor2, ShortTitle,ID_FOR,date_id2);
		}
		$('.modal.in').modal('hide');		
		}, 2000);
...
switch(appendType){
	case 'Holiday'		 : appendClass='HolidayParameterSet '; ContainerDate = $("#HijriDateBox").val(); break;
	case 'Holiday_Hijri' : appendClass='HolidayParameterSet'; ContainerDate = $("#HijriDateBox").val(); break;
	case 'StaffP'	: appendClass='StaffParameterSet';  ContainerDate = $("#date_id_SP").val(); break;
	case 'StaffPH'	: appendClass='StaffParameterSet Staff_Hijri';  ContainerDate = $("#date_id_SP").val(); break;
	case 'StudentP' : appendClass='StudentParameterSet'; ContainerDate = $("#HijriDateBox").val(); break;
	case 'Schedule' : appendClass='ScheduleFollowupSet EditSchedule'; ContainerDate = $("#ScheduleFCurrentDate").val(); break;
	case 'AdEnt' 	: appendClass='ScheduleFollowupSet EditAddEvent'; ContainerDate = $("#AdNwEnt").val(); break;
	case 'AdEntH' 	: appendClass='ScheduleFollowupSet EditAddEventH'; ContainerDate = $("#AdNwEnt").val(); break;
	case 'AdEntH2' 	: appendClass='ScheduleFollowupSet EdtAdEntH'; ContainerDate = $("#AdNwEnt").val(); break;
}
 //"View 10 more..";
var AnchorID = "#"+sdate;
...
if( anchor < 5  ){
	if(anchor == 4 ){
		EL.append('<a class="'+appendClass+'" href="#" id="'+lastID+'" data-toggle="modal" data-target="#SetStaffParameter" data-id="'+ContainerDate+'">'+ShortTitle+'</a>');
		var n1 = anchor+1;
...
switch(appendType){
	...
	case 'Holiday_Hijri' : 
		appendClass='HolidayParameterSet3 HlP'; 
		HijriDateBox = $("#HijriDateBox").val(); 
	break;
...
if( anchor < 5  ){
	if(anchor == 4 ){
		EL.append('<a class="'+appendClass+'" href="#" id="'+lastID+'" data-id="'+HijriDateBox+'" data-toggle="modal" data-target="#SetStaffParameter">'+ShortTitle+'</a>');
...
function AddEventDatabase(date_id){
	...
	$.ajax({
		...
		success: function(res){
			$("#AjaxReturnedID").val(0);
			$("#AjaxReturnedID").val( res.lId );
			Update_Weekly_Time_Sheet(AdNwEntDate, AdNwEntDate);
		},
		complete: function (){  }
	});
...
$(document).on("click",".EditAddEvent", function(){
	...
	loadModal('AddEvent',ID,'EdtAdEnt', Box_Dated); // EHP for new edit holiday parameter
...
var i =ID.split("_");
var ID = parseInt(i[1]);
var Hijri_Date = $(this).data("id");

loadModal('Schedule',ID, 'EdtSch', Hijri_Date  ); // EHP for new edit holiday parameter
...
$(document).on("click",".EdtAdEntH", function(){
	...
	loadModal('AddEventHijri',ID,'EdtAdEntH', Box_Dated ); // EHP for new edit holiday parameter
