...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // View, Edit, Delete from Table (Admission) *** only VIEW
            <?php if($this->data['can_user_view'] == 1 && $this->data['can_user_delete'] == 0 &&   $this->uri->uri_string() == 'assestment/term' ) {?>
                
            if ($('#assessment_term_view_table').length) {
                var oTable = $('#assessment_term_view_table').dataTable({
                    "oLanguage": {
...
if($('#order-form').length){
    $('#order-form').validate({
        ...
        if(data != 1)
        {
            ...
            $('#alert_validation').fadeOut(15000);
