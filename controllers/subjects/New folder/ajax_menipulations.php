...
class Ajax_menipulations extends CI_Controller{
  ...
  public function childLi($parentID , $su = NULL,$opt = NULL ) {
	...
	public function programmOptions(){
		...
		//grd_1- std_6023 -grp_1
		$unique = $jsonDecoded["unique"];
		$grpSbID = $jsonDecoded["grpSbID"]; // selected option programme
		...
		/*$data1 = array( "grdStdGrp"=>$unique, "subject_selection_grade_id"=>$grpSbID );
		
		$select1 = "subject_selection_grade_id AS PreID";
		$preID = 0;
		if( $previousRow = $this->SM->getSingleRow( $table, $select1, $data1=NULL   ) ){
			$preID = $previousRow->PreID;	
		}else{ $preID = 0;  } */
		...
		//SELECT * FROM `subject_selection_student2` WHERE `grade_id`=1 AND `group_no` =1 AND subject_selection_grade_id =12 
		$data3 = array( "grade_id" => $GradeID, "group_no"=>$group_no, "subject_selection_grade_id" => $grpSbID);
		$thisSubject = $this->SM->countData( $table, $data3 );
		
		
		$data4 = array( "grade_id" => $GradeID, "group_no"=>$group_no, 'subject_selection_grade_id != ' => 0);
		$groupTotal = $this->SM->countData( $table, $data4 );
		
		$encode = array( "thisSubject"=>$thisSubject,"groupTotal"=>$groupTotal);
	...
	public function grdGrpSubjectsUlFormate($grdID, $grpID,$countGroup,$acadmic,$noOfGroup,$term,$totalStudent){
		...
		if( $grdGrpSubID = $this->SM->getR( $tablename, $select, $where) ){
			...
			foreach( $grdGrpSubID as $sID ){
				...
				$subids = $this->SM->join(  $tbl2, $tablename, $onOpt, $select2, $where2 );
$where3 = array( "subject_selection_grade_id" => $sID["subjectID"], "grade_id" =>$grdID, "group_no"=>$noOfGroup, 'subject_selection_grade_id >' => 0,
"session"=>$acadmic, "term"=>$term );
