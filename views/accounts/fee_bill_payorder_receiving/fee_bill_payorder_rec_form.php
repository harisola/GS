    ...
    <div class="powerwidget <?php echo $title_background_color_add; ?>" id="fee_bill_payorder_receiving_div2" data-widget-editbutton="false" role="widget">
      ...
      <div class="inner-spacer" role="content">
        <form action="<?php echo site_url()?>/accounts/fee_bill_payorder_receiving" id="new_student_form" name="new_student_form" class="orb-form" novalidate="novalidate" method="POST">
            ...
            <fieldset>
              <section>
                <label class="input"> <i class="icon-append fa fa-user"></i>
                  <input type="text" name="fee_bill_no" id="fee_bill_no" placeholder="<?php echo date('y') . '-01-GSID-9'; ?>" autofocus="autofocus" required="required" value="<?php if(validation_errors() != false) { echo $this->input->post('fee_bill_no'); } ?>">
          ...
          <footer>
            <button type="submit" <?php if ($freeze=="Yes"){ echo"disabled"; } ?> name="fo_new_admission_form_button" class="btn btn-orb-org">Proceed</button>
          </footer>
        </form>
      </div>
    </div>
