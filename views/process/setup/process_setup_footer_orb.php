...
$(document).on("keyup input", ".marks", function (event){
	
	var a_marks = $(this).val(); // Assign Marks to Student
	...
	var charCode2 = event.keyCode || event.which || event.charCode;
	if( charCode2 == 8 ){
...
//a_marks <= max_marks
//faCheck.hide("slow");
var n_value = $(this).val(); // after dot
if( n_value != '' ){
if ( ( charCode2 >= 48 && charCode2 <= 57)  || ( charCode2 >= 96 && charCode2 <= 105) || charCode2 == 110 || charCode2 == 8 || charCode2 == 190 ||  $.type(charCode2) === "undefined"  ) {
...
faCheck.hide("slow");
	...
	if( validNumber != 0 ){
		...
		//var n_value = $(this).val(); // after dot
		var n_value = this.value;
		
		if( ( parseFloat(  n_value ) <= parseFloat(max_marks) ) ){
			var return_response = sendAjax(detail_id,user_ip,user_agent,a_marks,student_id,user_id,pending_marks,n_value );
			setTimeout(function(){
				triangle.hide("slow"); // Red Triangle For Error
				invalid2.hide("slow");
				pencil.hide("slow");
				input.removeClass("state-error");
				loadingIcon.hide("slow");
				faCheck.show("slow");	
				if(return_response == 1){ invalid3.show("slow"); }else{ invalid3.hide("slow"); }
			
			},1000);
		}else{
		var return_response = sendAjax2(detail_id,user_ip,user_agent,a_marks,student_id,user_id,pending_marks,mObtn );
			setTimeout(function(){
				loadingIcon.hide("slow");
				faCheck.hide("slow");	
				pencil.show("slow");
				triangle.show("slow"); // Red Triangle For Error
				invalid2.show("slow");
				invalid.hide("slow");
		...
		pencil.hide("slow");
		triangle.hide("slow"); 		// Red Triangle For Error
		invalid2.hide("slow"); 		//  Marks Increased Not Assigned
		invalid.show("slow");   // Show Enter Valid Number	
		},1000);
	...
	// chech 
	if(  $(this).val() != '' ){
	setTimeout(function(){ faCheck.show("slow");	},1000);
	}else{
	setTimeout(function(){ faCheck.hide("slow"); },1000);
	}
}
