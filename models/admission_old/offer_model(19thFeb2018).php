...
class Offer_model extends CI_Controller{
	...
	public function offer_detail_by_grade($grade_id){
		...
		WHERE af.grade_id = $grade_id AND af.form_status_id >= 5 AND (af.form_status_stage_id = 3 OR af.form_status_stage_id = 1 OR af.form_status_stage_id = 10 /*OR af.form_status_stage_id = 12 */ OR af.form_status_stage_id = 2)";
		$row = $this->db->query($query);
		return $row->result();
	}
	...
	public function get_OfferLetter_StudentData($FormNo){
		...
		$query = "Select
					...
					DATE_FORMAT('".$DueDate."','%W, %M %d, %Y') as valid_date,
					CONCAT( CASE  
	...
	****************/
	...
	public function getSiblingsInfo($GFID)
	{
		...
		$cQuery = "
			...
			'' as house_red, '' as house_green, '' as house_blue, '' as house_color_hex, '' as academic_session_id, '' as std_status_code
			from atif_gs_admission.admission_form as af where gf_id = $GFID
			group by gs_id 
			order by grade_id desc";
		} else {

			$cQuery = "select * from atif.students_all where gf_id = $GFID 
				union
				select '99999' as student_id, '99999' as gr_no, af.form_no as gs_id, af.gf_id, af.official_name, '' as abridged_name,
				af.call_name, af.gender, af.dob, '' as nationality_1, '' as nationality_2, '' as religion, 2018 as year_of_admission,
				af.grade_name as grade_of_admission, '' as previous_school, '' as mobile_phone, '' as email, '' as facebook, '' as linkedin,
	...
	public function offer_detail_by_grade_a_level($grade_id){
		...
		af.official_name as applicant_name, af.father_name,
		IFNULL(DATE_FORMAT(af.offer_date, '%a %d, %b %Y'),'') as offer_date,
		IFNULL(DATE_FORMAT(of.due_date, '%a %d, %b %Y'),'') as payment_due_date,
		...
		IF(af.father_qualification='' AND af.father_occupation='' AND af.father_department='' AND af.father_company='' AND af.father_office_location='' AND af.father_work_phone='' AND af.mother_qualification='' AND af.mother_department='' AND af.mother_company='' AND af.mother_office_location='' AND af.mother_work_phone='' AND af.mother_occupation='' , 0, 1) as is_father_mother,
		...
		where af.grade_id = $grade_id and af.form_status_id >= 5 and (af.form_status_stage_id = 3 OR af.form_status_stage_id = 1 OR af.form_status_stage_id = 10 OR af.form_status_stage_id = 12 OR af.form_status_stage_id = 2) and af.form_discussion_criteria = 'EAO'";
		$row = $this->db->query($query);
		return $row->result();
	}
