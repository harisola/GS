...
<script type="text/javascript">
$(document).ready(function() {
    $(document).on("click","#get_grade_classes",function() {
        event.stopPropagation();


        var selected_grade = $('#selected_grade').val();
        var official_name = "";
        var call_name = "";
        var dob = "";
        var gender = "";
        ...
        if($("input[name='official_name']:checked").length > 0){ official_name = "yes"; } else { official_name = ""; }
        if($("input[name='call_name']:checked").length > 0){ call_name = "yes"; } else { call_name = ""; }
        if($("input[name='dob']:checked").length > 0){ dob = "yes"; } else { dob = ""; }
        if($("input[name='gender']:checked").length > 0){ gender = "yes"; } else { gender = ""; }

        if($("input[name='age']:checked").length > 0){ age = "yes"; } else { age = ""; }
        if($("input[name='gf_id']:checked").length > 0){ gf_id = "yes"; } else { gf_id = ""; }
        if($("input[name='house']:checked").length > 0){ house = "yes"; } else { house = ""; }
        if($("input[name='previous_grade']:checked").length > 0){ previous_grade = "yes"; } else { previous_grade = ""; }

        if($("input[name='siblings']:checked").length > 0){ siblings = "yes"; } else { siblings = ""; }
        if($("input[name='father_name']:checked").length > 0){ father_name = "yes"; } else { father_name = ""; }
        if($("input[name='mother_name']:checked").length > 0){ mother_name = "yes"; } else { mother_name = ""; }
        if($("input[name='petitioner_code']:checked").length > 0){ petitioner_code = "yes"; } else { petitioner_code = ""; }

        if($("input[name='blank']:checked").length > 0){ blank = "yes"; } else { blank = ""; }


        
        var url = "selected_grade="+selected_grade+"&";
        var url = url + "official_name="+official_name+"&";
        var url = url + "call_name="+call_name+"&";
        var url = url + "dob="+dob+"&";

        var url = url + "gender="+gender+"&";
        var url = url + "age="+age+"&";
        var url = url + "gf_id="+gf_id+"&";
        var url = url + "house="+house+"&";

        var url = url + "previous_grade="+previous_grade+"&";
        var url = url + "siblings="+siblings+"&";
        var url = url + "father_name="+father_name+"&";
        var url = url + "mother_name="+mother_name+"&";

        var url = url + "petitioner_code="+petitioner_code+"&";
        var url = url + "blank="+blank;


        var win = window.open("<?php echo site_url() . '/class_list/name_view/printForm2?';?>"+url, '_blank');
...
if ($('#Session_Name').length) {
    $('#Session_Name').mask('2099-2099', {
        placeholder: 'X'
    });
}

if ($('#Display_Name').length) {
    $('#Display_Name').mask('2099-99', {
        placeholder: 'X'
    });
}

$(document).on("click", "#btn_new_session", function(){


$.validator.addMethod("notequal", function(value, element) {
 return $('#Start_Date').val() != $('#End_Date').val()}, 
 "* You've entered Same Dates ");
...
{ 
    ...
    Start_Date: { 
        required: true, 
        remote: "<?=base_url();?>index.php/accounts/structure_ajax/CheckStartDate"
        },
    End_Date: { 
        required: true, 
        notequal: true, 
        greaterthen:true,
        remote: "<?=base_url();?>index.php/accounts/structure_ajax/CheckEndDate"
    },
},
messages: 
{
    Session_Name :{ required: 'Please Mention Session Name!' },
    Display_Name :{ required: 'Please Mention Display Name!' },
    Start_Date :{ 
        required: 'Please Mention Start Date!',
        remote: "Wronge Selection. Please Select other Date." 
    },
    End_Date :{ 
        required: 'Please Mention End Date!', 
        remote: 'End Date Wonge Selection. Please Select other Date.' 
    },
},
...
// }// Call Back
// },//end Confirm
// cancel: { label: "Cancel", callback: function() { RCBack = 0; } },
...
$("#fee_definition_active").validate({
rules: 
{ 
    ignore: [],
    Hidden_Definition_id : { required: true },
},
messages: 
{
    Hidden_Definition_id :{ required: 'Please Mention Session Name!' },
