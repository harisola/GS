...
class Fee_bill_upload_mis extends MY_Controller {
		...
		if(isset($_POST["submit"]))
		{
			...
			{	
				...
				for ($i=$titleRow+1; $i<=$totalRecords; $i++){
					...
					if(substr(trim($data['values'][$i]['G']), 2, 1) != 0){
					...
					if(substr($thisGBID, 2, 1) != 0){
						if(strlen($data['values'][$i]['G']) != 9 || $this->fee_bill_upload_mis_model->checkBillID($thisGBID) == false ||
							...
							$lastBillNumber != $thisGBID
						   	...
						   	}else if(strlen($data['values'][$i]['G']) != 9){
					...
					$correctFeeAmount = floatval($this->fee_bill_upload_mis_model->getCorrectFeeAmount($lastBillNumber));
				...
				if($uploadOk == 1){
					for ($i=$titleRow+1; $i<=$totalRecords; $i++){
						...
						if($FeeBillReceivingID == 0){
							...
							/******************************************************/
							/* Student Registration is here is Admission Fee paid */
							/******************************************************/
							if(substr(trim($data['values'][$i]['G']), 2, 1) == 0){
	...
	public function RegisterThisStudent($FeeBillID, $ReceivedDate, $ReceivedBranch){
			...
			$FormNo = substr($FeeBillID, 4, 1) . substr($FeeBillID, 5, 3);
