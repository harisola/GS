...
class Viewtifb2 extends CI_Controller {
	...
	public function index(){
		...
		$this->load->model('tif_model/tif_form_model');
			...
			$gt_id = $this->tif_form_model->getAllStaffInfo();

			$urlPDF = base_url()."index.php/staff_information_form_basic/viewtifb2/makePDFFile?download=1&solangigtID=";

			foreach($gt_id as $g)
			{
				$solangigtID = $g["gt_id"];
				$fileURL = $urlPDF.$solangigtID;
				echo '<script type="text/javascript">   window.open("'.$fileURL.'", "_blank"); </script>';
			}	
				
			
			//$this->load->view('staff_information_form/fif_form_id3');
	...
	public function add(){
		...
		$schoolGrade = $this->input->post('schoolGrade');
		...
		$collegeGrade = $this->input->post('collegeGrade');
		...
		$universityGrade = $this->input->post('universityGrade');
		...
		$professionalGrade = $this->input->post('professionalGrade');
		...
		$othersGrade = $this->input->post('othersGrade');
	...
	public function makePDFFile(){
		
	// 			ini_set('display_errors', 'on');
	// error_reporting(E_ALL | E_STRICT);
		$solangigtID = $this->input->get('solangigtID');
	...
	if( !empty( $employeSupporting[0]->nationality) ){
		...
		//Child Detail
		if( (!empty($gfid)) && ( !empty($data['child_detail'])))
		{
...
// 					} 
				    ...
				    $pdf->Cell(53, 6.5, $Email, $borderOn, 0, 'L', 0);
		...
		if(isset($fileMode) && $fileMode == "1"){
            $pdf->Output($solangigtID . '-' . $FullName . '.pdf', 'D');
        }else{
