...
class Student_profile_basic_view_absent extends CI_Controller {
      ...
      if($data['siblings_data'][0]['siblings_total'] > 1){
        ...
        foreach ($data['siblings_data'] as $sibling) {
          ...
          if($sibling['gr_no'] == $StudentGRNO){
            ...
            $data['attendance'] = $this->std_atd_change_model->getStudentAttendance($sibling['gs_id']);
                  
                
            
            /*if(!isset($data['attendance'][0])){
              $siblings_block .= '
                <h3 id="abridged_name" style="background-color:#FF4D4D; color:white">' . $sibling['abridged_name'] . '</h3>';
            }else{
              $siblings_block .= '
            ...
            if(!isset($data['attendance'][0])){
              $siblings_block .= '
                <li style="background-color:#FF4D4D; color:white">Absent</li>';
            }else{
              $siblings_block .= '
                <li>' . $data['attendance'][0]['time'] .'</li>';
            }
