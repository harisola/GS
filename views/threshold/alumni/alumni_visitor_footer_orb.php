...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            <?php if($this->data['can_user_view'] == 1 && $this->data['can_user_delete'] == 0 && $this->uri->uri_string() == 'threshold/visitor_alumni') {?>
            if ($('#alumni_students_visit_today').length) {
                var oTable = $('#alumni_students_visit_today').dataTable({
                    "oLanguage": {
...
/*$(".student_gsid").click( function()
   {      
     $("#gs_id").val($(".student_gsid").val());     
     $('#gs_id').focus();
   }
);*/
...
$("#review-form").validate({
	...
	messages: {
		gs_id: { required: 'Please enter GS-ID' },
		alumni_assigned_card: { required: 'Please enter card' }
...
$(document).on("click",".student_gsid", function (){
	
	var student_gsid = $(this).val();
	
	//console.log(student_gsid);
	$("#gs_id").val( student_gsid );
	...
	var ssname = $mainRow.find(".abridged_name");
	var ssname = ssname.text();
	//console.log( ssname );
	$("#abridged_name").val("");
	$("#abridged_name").val(ssname);
