...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // ========================================================================        
        if ($('#visitor_admission_assignedcard_form').length) {
            $("#visitor_admission_assignedcard_form").validate({
                ...
                messages: {
                    cnic: {
                        required: 'Please enter guest NIC'
                    },
                    name: {
                        required: 'Please enter guest Name'
                    },
                    mobile_phone: {
                        required: 'Please enter guest Mobile Phone'
                    }
                },
...
$(document).ready(function(){
    $('[name=visitor_admission_assignedcard_reset_button]').click(function()  {        
		reset();
	});
});

/*
$(document).on("keyup", "#admission_assigned_card",function(){
...
$("#review-form").validate({
	...
	rules: {
		...
		admission_visit_purpose: { required: true },
		admission_visit_to_dapartment: { required: true },
		admission_assigned_card: { 
		required: true,
			number: true,
			minlength: 10,
			maxlength: 10
	...
	// Messages for form validation
	messages: {
		cnic: { required: 'Please enter CNIC' },
		name: { required: 'Please enter Name' },
		mobile_phone: { required: 'Please enter Contact Number' },
		admission_visit_purpose: { required: 'Please enter Visiting Purpose' },
		admission_visit_to_dapartment: { required: 'Please enter Visiting Department' },
		admission_assigned_card: { required: 'Please enter Card Number'}
	},
	// Ajax form submition					
...
function reset(){
	 $('[name=cnic]').val("");
        $('[name=name]').val("");
        $('[name=mobile_phone]').val("");
        $('[name=admission_visit_purpose]').val("");
        $('[name=admission_visit_to_person]').val("");
        $('[name=admission_visit_to_dapartment]').val("");
        $('[name=admission_assigned_card]').val("");
        $('#cnic').focus();
}
