...
var oTable = $('#table_container').dataTable({
	...
	// "sDom": '<"search"fTlp><"bottom"><"clear">lrtip',
	// "sDom": 'T<"clear">lfrtip',
	// tableTools: {  "sSwfPath": "<?php echo base_url()?>components/js/swf/copy_csv_xls_pdf.swf" },
	// "bProcessing": true,
	// "bServerSide": true,
	// "sScrollX": "100%",
	// "bScrollCollapse": true,
	...
	// "jQueryUI": true
	 ...
	 $('#msds_select').on('change',function(){
...
$(document).on("click", ".open_show", function(){
	...
	if( notify_id > 0 )
	{
		$.ajax({
			...
			data: {notify_id:notify_id,cat_id:cat_id,staff_id:staff_id,emp_id:emp_id,gs_id:gs_id,notify_to:notify_to,current_user:current_user,notify_type:notify_type},
			dataType: "JSON",
			beforeSend: function(){
			},
			...
			$.each( obj, function( index, value ) {
				...
				if( $("#admission_counter").length )  
				{
					var ACouter = parseInt( $("#admission_counter").html() );
					var sum = (ACouter - 1);
					
					if( sum > 0 ){
					$("#admission_counter").html(sum);
					}
					else
					{
						$("#admission_counter").hide();
						$("#admission_counter_p").hide();
...
function Admission_Process(Form_id)
{
